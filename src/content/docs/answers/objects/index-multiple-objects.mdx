---
description: The Tallyfy Answers batch import API lets you index multiple objects
  into a collection through a single POST request using a JSON array format which
  is far faster than individual calls and runs large imports in the background with
...
Large batch imports run in the background. You can monitor progress and cancel operations if needed using the [Tasks](/products/answers/tasks/) endpoints.
