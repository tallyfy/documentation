---
description: The Denizen API automatically detects user location via IP address and
  returns appropriate localized images with city-level coverage for the United States
  and country-level coverage for other regions using Cloudflare's global infrastructure
  for high-performance delivery without requiring authentication.
