...
sequenceDiagram
    ...
    participant WS as WebSocket Server<br/>(collector.tallyfy.com)
    
    Note over C: Has client_uuid and<br/>actor_profile_id from<br/>prior authentication
    ...
    C->>WS: Send auth message<br/>{type: "auth", org_id, project_id,<br/>client_uuid, actor_profile_id}
    WS->>WS: Validate credentials
    WS-->>C: {type: "auth", status: "ok"}
    
    Note over C,WS: Connection authenticated<br/>Ready to send events
```
