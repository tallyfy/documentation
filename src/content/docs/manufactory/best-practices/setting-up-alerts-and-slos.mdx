---
description: Service Level Objectives (SLOs) provide a user-centric framework for
  creating actionable alerts based on error budget burn rates rather than traditional
  cause-based alerting which reduces noise and alert fatigue while ensuring meaningful
  notifications when system reliability targets are at risk.
...
```mermaid
flowchart TD
 Events[Event Data] --> SLI[Calculate SLI]
 SLI --> Budget{Error Budget Burn Rate?}
 
 Budget -->|"Fast: 4h"| Critical["🚨 Critical Page On-Call"]
 Budget -->|"Slow: 24h"| Warning["⚠️ Warning Notification"]
 Budget -->|"Normal"| OK["✅ Within Budget"]
 
 Critical --> Investigate["Investigate Issue"]
 Warning --> Monitor["Monitor Closely"]
 OK --> Continue["Continue Normal Ops"]
 
 Investigate --> Fix["Apply Fix"]
 Monitor --> Fix
 Fix --> Events
 
 click SLI "/products/manufactory/best-practices/what-is-observability/" "Learn about observability"
 click Fix "/products/manufactory/actors/index/" "Understanding Manufactory actors"
```
...
<LinkTitleCard header="<b>Best Practices > Analyzing events and deriving insights</b>" href="/products/manufactory/best-practices/analyzing-events-and-deriving-insights/" > Event analysis in Tallyfy Manufactory transforms raw event data into actionable insights through systematic investigation using filtering grouping and correlation techniques to understand system behavior troubleshoot issues and drive continuous improvements. </LinkTitleCard>
