---
description: SLOs provide a more effective alerting framework for Tallyfy Manufactory
  event-driven systems by focusing on user-centric reliability targets and error budget
  burn rates rather than traditional cause-based alerts that often create noise and
  alert fatigue.
...
flowchart TD
    ...
    SLI --> Budget{Error Budget<br/>Burn Rate?}
    
    Budget -->|"`Fast: 4h`"| Critical["`**🚨 Critical**
Page On-Call`"]
    Budget -->|"`Slow: 24h`"| Warning["`**⚠️ Warning**
Notify Team`"]
    Budget -->|"`Normal`"| OK[✅ Monitor]
    
    Critical --> Fix["`**Investigate**
& Fix`"]
    Warning --> Fix
...
<LinkTitleCard header="<b>Best Practices > Analyzing events and deriving insights</b>" href="/products/manufactory/best-practices/analyzing-events-and-deriving-insights/" > Event analysis transforms raw data from instrumented applications into actionable insights by systematically investigating what happened why it occurred and its impact through an iterative process of observing orienting deciding and learning to understand system behavior troubleshoot issues and drive continuous improvements. </LinkTitleCard>
