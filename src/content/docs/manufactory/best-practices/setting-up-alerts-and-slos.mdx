...
```mermaid
flowchart TD
    Start([Event Data from<br/>Manufactory]) --> SLI[Calculate SLI<br/>e.g., Success Rate]
    
    SLI --> Compare{Compare to<br/>SLO Target}
    
    Compare -->|Below Target| Consume[Consume Error Budget]
    Compare -->|Meets Target| OK[Within Budget]
    
    Consume --> BurnCalc[Calculate Burn Rate<br/>over Time Windows]
    
    BurnCalc --> FastBurn{Fast Burn?<br/>Budget gone in 4h}
    BurnCalc --> SlowBurn{Slow Burn?<br/>Budget gone in 24h}
    
    FastBurn -->|Yes| Critical[🚨 Critical Alert<br/>Page On-Call]
    FastBurn -->|No| CheckSlow
    
    SlowBurn -->|Yes| Warning[⚠️ Warning Alert<br/>Notify Team]
    SlowBurn -->|No| Monitor[Continue Monitoring]
    
    CheckSlow[Check Slower Burns] --> SlowBurn
    
    Critical --> Investigate[Investigate Actor/<br/>Event Issues]
    Warning --> Investigate
    ...
    style OK fill:#D4EDDA,stroke:#00AA55
    
    click SLI "/products/manufactory/best-practices/what-is-observability/" "Learn about observability"
    click Investigate "/products/manufactory/actors/" "Understanding Manufactory actors"
```
