---
description: Tallyfy Manufactory lets you turn raw ingested event data into practical
  insights by following an iterative analysis loop of observing issues and forming
  hypotheses then filtering and grouping events by attributes like type or source
  or error code to identify root causes and drive improvements in workflow design
  and actor logic and system performance.
...
Once your applications are instrumented to send rich, structured events to Tallyfy Manufactory, your next step is turning that raw data into useful insights. Good event analysis helps you understand the behavior of your event-driven systems, troubleshoot issues, and continuously improve your processes.
...
While Tallyfy Manufactory is an event ingestion and lifecycle engine, it may offer features for basic event searching and viewing. For deeper analysis, you might export event data from Manufactory (if available) or make sure events also go to dedicated observability or analytics platforms[^1] where you can run more advanced queries.

## The core analysis loop for event data

A systematic approach to analyzing event data, often called the "core analysis loop," works well for investigating issues with Tallyfy Manufactory event processing:
...
This diagram shows the cyclical nature of event analysis, moving from observation through hypothesis testing to useful insights.
...
Start: "Start"
Test: "Test"
Start: Event Issue/Alert in Manufactory {
  shape: oval
}
Filter: Filter Relevant Events {
  shape: rectangle
}
...
Check -> Action: Yes
Check -> Refine: No
Check -> Analyze: Partial
Refine -> Test
```

**What to notice:**
- The process is iterative, so you may need several cycles to reach the root cause
...
Here are typical questions you can answer by analyzing event data from Tallyfy Manufactory:
...
To answer these questions, you'll use various analytical techniques on your event data (whether directly in Manufactory or in an external platform):
...
*   **Anomaly Detection (Conceptual)[^2]:** This means identifying unusual patterns or significant deviations from normal behavior in your event streams. While complex anomaly detection might require specialized tools, even simple visual inspection of time-series graphs of Manufactory event volumes can reveal anomalies.

## Using Tallyfy Manufactory's features for analysis

How you analyze event data depends on Tallyfy Manufactory's specific capabilities. Consider:
...
Understanding these features helps you find the best way to analyze data from Manufactory's operations.
...
Analyzing event data from Tallyfy Manufactory is a skill you'll develop over time:
...
*   **Share your findings and techniques** with your team to build shared knowledge and find new ways to use Manufactory data.
...
[^1]: Common platforms include Datadog, New Relic, Splunk, or Elastic Stack for event stream analysis
[^2]: Statistical methods like z-scores or isolation forests can identify outliers in event patterns
...
<LinkTitleCard header="<b>Manufactory > Introduction to observability best practices</b>" href="/products/manufactory/best-practices/" > This guide explains how observability practices enable deep understanding of event-driven... </LinkTitleCard>
...
<LinkTitleCard header="<b>Best Practices > Adopting an observability culture</b>" href="/products/manufactory/best-practices/adopting-observability-culture/" > Building an observability culture for Tallyfy Manufactory requires fostering team-wide curiosity... </LinkTitleCard>
</CardGrid>