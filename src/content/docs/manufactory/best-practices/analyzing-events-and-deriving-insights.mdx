...
```mermaid
flowchart TD
    Start([Event Issue/Alert<br/>in Manufactory]) --> Observe[1. Observe<br/>High-level view of problem]
    
    Observe --> Orient[2. Orient<br/>Formulate hypothesis]
    
    Orient --> Decide[3. Decide & Act<br/>Query and filter event data]
    
    Decide --> Filter[Filter by:<br/>• Time ranges<br/>• Event types<br/>• Field values]
    
    Filter --> Group[Group & Aggregate:<br/>• By dimensions<br/>• Count events<br/>• Calculate metrics]
    
    Group --> Learn{4. Learn<br/>Does it confirm<br/>hypothesis?}
    
    Learn -->|Yes| Answer[Root Cause<br/>Identified]
    Learn -->|No| Refine[Refine hypothesis<br/>or query]
    
    Refine --> Orient
    Learn -->|Partially| Deeper[New specific<br/>questions]
    Deeper --> Observe
    
    Answer --> Action[Drive<br/>Improvements]
