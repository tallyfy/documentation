---
description: Distributed tracing tracks single events as they cascade through multiple
  services in complex event-driven architectures by creating unique identifiers and
  hierarchical relationships that visualize the complete journey from initial trigger
  through processing stages like Tallyfy Manufactory ingestion routing and actor execution.
...
UAS: UserAccountService (Span A) {
  shape: rectangle
}
M: Manufactory Ingestion (Span B) {
  shape: rectangle
}
P: Manufactory Project (Span C) {
  shape: rectangle
}
WE: WelcomeEmail Actor (Span D) {
  shape: hexagon
}
PT: ProvisionTrial Actor (Span E) {
  shape: hexagon
}

User -> UAS: "1. Signup Request"
UAS.note: |md
  Create Span A (Root)
  Trace ID: xyz123
  2. Create User Account
|
UAS -> M: "3. UserSignedUp Event + Trace Context\n(TraceID: xyz123, ParentID: A)"

M.note: |md
  Create Span B
  Parent: Span A
|
M -> P: "4. Route to Onboarding Project\n(TraceID: xyz123, ParentID: B)"
...
parallel: Parallel Actor Execution {
  style.stroke: "#FF6B6B"
  style.stroke-width: 2
  style.stroke-dash: 3
  
  P -> WE: "5. Trigger WelcomeEmailActor\n(TraceID: xyz123, ParentID: C)"
  WE.note: |md
    Create Span D
    Parent: Span C
    6. Send Email
  ...
  P -> PT: "5. Trigger ProvisionTrialActor\n(TraceID: xyz123, ParentID: C)"
  PT.note: |md
    Create Span E
    Parent: Span C
    6. Grant Access
...
footnote: |md
  Complete Trace: Spans A→B→C→(D,E)
|
```
