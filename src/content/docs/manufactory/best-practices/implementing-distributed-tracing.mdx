---
description: Distributed tracing tracks a single event's complete journey across multiple
  services and components in complex architectures by creating connected spans with
  unique identifiers that visualize the entire workflow from initiation through processing
  to final outcomes.
