...
sequenceDiagram
    autonumber
    participant User
    participant UAS as UserAccountService<br/>(Span A)
    participant M as Manufactory<br/>Ingestion<br/>(Span B)
    participant P as Manufactory<br/>Project<br/>(Span C)
    participant WE as WelcomeEmail<br/>Actor<br/>(Span D)
    participant PT as ProvisionTrial<br/>Actor<br/>(Span E)
    
    User->>UAS: Signup Request
    Note over UAS: Create Span A (Root)<br/>Trace ID: xyz123
    UAS->>UAS: Create User Account
    UAS->>M: UserSignedUp Event<br/>+ Trace Context<br/>(TraceID: xyz123, ParentID: A)
    
    Note over M: Create Span B<br/>Parent: Span A
    M->>P: Route to Onboarding Project<br/>(TraceID: xyz123, ParentID: B)
    
    Note over P: Create Span C<br/>Parent: Span B
    
    par Parallel Actor Execution
        P->>WE: Trigger WelcomeEmailActor<br/>(TraceID: xyz123, ParentID: C)
        Note over WE: Create Span D<br/>Parent: Span C
        WE->>WE: Send Email
    and
        P->>PT: Trigger ProvisionTrialActor<br/>(TraceID: xyz123, ParentID: C)
        Note over PT: Create Span E<br/>Parent: Span C
        PT->>PT: Grant Access
    end
