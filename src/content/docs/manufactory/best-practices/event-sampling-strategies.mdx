---
description: Event sampling strategies for Tallyfy Manufactory let you reduce data
  volume and cost by selectively keeping the most important events—such as errors
  and critical business transactions—while discarding or down-sampling repetitive
  low-value ones through fixed-rate or adaptive approaches that still preserve enough
  signal for effective debugging and system understanding.
id: f0df140cd0abe1af3910a9af9b15fe01
lastUpdated: 2026-03-22
sidebar:
  order: 7
title: Event sampling strategies
---

As your systems scale, the volume of events can grow fast. Tallyfy Manufactory handles large event loads, but sending, storing, and processing every single event can become impractical or cost-prohibitive for both Manufactory and downstream observability or analytics systems. Event sampling lets you manage this volume while keeping the data you need for good observability.

## Why consider sampling for events sent to Tallyfy Manufactory?

Implementing an event sampling strategy before events reach Tallyfy Manufactory, or for data exported from Manufactory, has several benefits:
...
*   **Performance Optimization:** Ingesting, indexing, and analyzing massive, unsampled event streams can affect the performance of Tallyfy Manufactory or the tools you use to query its data.
*   **Signal vs. Noise Enhancement:** Not all events are equally valuable for observability. Many events are repetitive and indicate successful, normal operations. While some record of these is important, keeping every single one isn't necessary if you keep a statistically representative sample. Sampling can help focus on the more interesting or anomalous events.

The main goal of sampling events for Tallyfy Manufactory is to reduce data volume without losing the ability to debug issues, understand system behavior, and meet your [Service Level Objectives (SLOs)](/products/manufactory/best-practices/setting-up-alerts-and-slos/) for processes involving Manufactory.
...
These sampling strategies are particularly relevant for events sent to or processed by Tallyfy Manufactory:
...
*   **Monitor Your Sampling Effectiveness:** Regularly ask: Can we still debug issues effectively with the sampled data? Are we missing important trends or anomalies in our Tallyfy Manufactory event flows because of sampling?

## How Tallyfy Manufactory might interact with sampled data

Consider these aspects regarding Tallyfy Manufactory itself:

*   **Awareness of `sample_rate`:** If events ingested by Manufactory include a `sample_rate` attribute, can Manufactory or any tools analyzing its data (or data exported from it) use this for weighted analysis to reconstruct accurate counts or metrics?
...
<LinkTitleCard header="<b>Best Practices > Best practices for instrumenting applications</b>" href="/products/manufactory/best-practices/instrumenting-applications-for-manufactory/" > Good instrumentation in Tallyfy Manufactory requires including unique identifiers along with... </LinkTitleCard>
<LinkTitleCard header="<b>Best Practices > Analyzing events and deriving insights</b>" href="/products/manufactory/best-practices/analyzing-events-and-deriving-insights/" > Effective event analysis in Tallyfy Manufactory transforms raw data into actionable insights... </LinkTitleCard>
