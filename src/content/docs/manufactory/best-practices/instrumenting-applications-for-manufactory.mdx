---
description: The quality and richness of event data you send to Tallyfy Manufactory
  directly determines how effectively it can route events to the right projects and
  trigger appropriate actors so investing in thoughtful instrumentation with unique
  identifiers and consistent field names and meaningful business context and tracing
  information pays off through better debugging and more precise workflow management.
...
Here are some universal principles for good instrumentation before getting into Tallyfy Manufactory specifics:

*   **Instrument for the data consumer:** Always think about who'll be using this event data and what questions they'll need to answer. This includes developers debugging issues, operations teams monitoring system health, and potentially product managers understanding feature usage through events processed by Manufactory.
...
*   **Iterate and refine:** Instrumentation isn't a one-time setup. As your systems and your understanding of them evolve, you'll likely identify new pieces of information that would be valuable to include in your events sent to Manufactory.
...
*   **Tracing information (if implementing distributed tracing[^1]):**
...
```json
{
  ...
  "timestamp_occurred": "2024-10-27T16:45:10Z",
...
*   **Including sensitive Personally Identifiable Information (PII)[^2]:** Ensure compliance with data privacy regulations. If sensitive data must be processed by Manufactory, understand its data handling policies and implement appropriate masking or tokenization upstream if necessary.
...
Thoughtful instrumentation isn't just an operational task. It's an investment that directly boosts the value you get from Tallyfy Manufactory by:
...
[^1]: A method to track requests across multiple services using trace_id, span_id to understand flow
[^2]: Data like SSN, email, or name that can identify individuals - requires special handling
...
<LinkTitleCard header="<b>Best Practices > Implementing distributed tracing for event workflows</b>" href="/products/manufactory/best-practices/implementing-distributed-tracing/" > Distributed tracing tracks the progression of events through complex microservices architectures... </LinkTitleCard>
