...
:::caution[Correction, 2026-09-16: the first item below no longer describes how Tallyfy behaves]
It was accurate when it was published and is left as written, because a changelog is a record of
what changed on a date.

Automations no longer re-run on a reopen. An automation now runs at most once in a given process,
so reopening a task, changing a field and completing it again doesn't fire it a second time. See
[when an automation runs](/products/pro/documenting/templates/automations/when-automations-run/)
for what does trigger one.
:::
