...
sidebar:
  order: 20
snippet: Learn how to handle idempotency when integrating with Tallyfy's webhooks and API to prevent duplicate actions and ensure reliable system behavior when events fire multiple times.
title: Handling idempotency in webhooks and API
---

## What is idempotency and why it matters

When integrating with Tallyfy's [webhooks](/products/pro/integrations/webhooks/) and [API](/products/pro/integrations/open-api/), you need to design your systems to handle events that might occur multiple times. *Idempotency* means that performing the same operation multiple times produces the same result as performing it once. This is essential for building reliable integrations.
...
2. Before processing a webhook, check if this combination already exists
...
1. Generate a unique event ID combining `task_id + event_type + timestamp`
...
3. **Implement request IDs**: Require external systems to send a unique request ID with each API call:
   ```
   X-Request-ID: unique-request-identifier-123
   ```
   Store these IDs temporarily (e.g., for 24 hours) to detect retries
...
2. **Use queues**: Implement a message queue to handle high volumes without overwhelming your system
...
1. **Implement idempotency keys**: Generate a unique key for each operation:
   ```
   POST /api/v1/processes/launch
   X-Idempotency-Key: ticket-12345-launch-attempt-1
   ```

2. **Use conditional requests**: Check if an action has already been performed:
...
3. **Verify data consistency**: Check that your data remains correct after duplicate events
4. **Monitor logs**: Look for patterns of duplicate events in production
...
1. Monitor your integration logs to identify patterns of duplicate events
2. Adjust your deduplication window based on actual retry patterns
3. Consider implementing more sophisticated event sourcing if you need full audit trails
4. Review Tallyfy's [webhook documentation](/products/pro/integrations/webhooks/) for the latest payload formats
