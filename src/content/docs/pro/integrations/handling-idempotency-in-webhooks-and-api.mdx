---
description: Idempotency ensures integrations handle duplicate events gracefully by
  preventing the same action from being processed multiple times even when webhooks
  fire repeatedly or external systems send duplicate requests.
