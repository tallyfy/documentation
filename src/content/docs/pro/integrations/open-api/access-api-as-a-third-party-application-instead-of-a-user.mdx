---
description: The OAuth 2.0 Client Credentials flow enables third-party applications
  to integrate with Tallyfy API by obtaining client credentials from support then
  following a secure authentication chain where applications first get app-level tokens
  to provision users and generate user-specific tokens allowing organizations to embed
  workflow functionality automate processes and manage users programmatically with
  proper security considerations and troubleshooting support.
...
sequenceDiagram
 ...
 participant Auth as Tallyfy Auth (account.tallyfy.com)
 participant API as Tallyfy API (go.tallyfy.com)
 participant User as End User
 
 Note over App,API: OAuth 2.0 Client Credentials Flow
 
 %% Step 1: Get App Token
 App->>Auth: POST /oauth/token client_id, client_secret grant_type: "client_credentials"
 Auth-->>App: Application Access Token (expires_in: 3600)
 
 %% Step 2: Provision User
 App->>API: POST /api/applications/{orgID}/users Bearer: {app_token} User details (name, email, role)
 API-->>App: User created (user_id: 12345)
 
 %% Step 3: Get User Token
 App->>API: POST /api/applications/{orgID}/users/{email}/token Bearer: {app_token}
 API-->>App: User-specific Access Token (expires_in: 3600)
 
 %% Step 4: Act on behalf of user
 App->>API: GET /api/organizations/{orgID}/me/tasks Bearer: {user_token} X-Tallyfy-Client: APIClient
 API-->>App: User's tasks data
 
 Note over App,User: User interacts with third-party app
 User->>App: Request workflow action
 App->>API: API call with user token
 API-->>App: Response
