...
sidebar:
  order: 20
snippet: The integration process enables third-party applications to access Tallyfy
  functionality through client credentials authentication managing user tokens and
  making API requests while following security best practices for enterprise-level
  implementations.
...
<LinkTitleCard header="<b>Open Api > OAuth authorization flow for third-party applications</b>" href="/products/pro/integrations/open-api/oauth-authorization-flow/" > A comprehensive guide detailing the steps to implement OAuth authorization flow for third-party applications to securely authenticate users with Tallyfy through client IDs redirect URIs and access tokens while following security best practices. </LinkTitleCard>
</CardGrid>