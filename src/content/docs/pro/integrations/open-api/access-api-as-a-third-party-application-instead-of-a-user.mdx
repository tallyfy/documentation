...
description: Third-party applications can integrate with Tallyfy using OAuth 2.0 client
  credentials flow to embed workflow functionality by obtaining client credentials
  from Tallyfy support then requesting application tokens to provision users and generate
  user-specific tokens for making API calls on behalf of users within their organization.
...
<LinkTitleCard header="<b>Open Api > OAuth authorization flow for third-party applications</b>" href="/products/pro/integrations/open-api/oauth-authorization-flow/" > OAuth authorization flow for third-party Tallyfy integrations enables secure user authentication through a standard process where users authenticate directly with Tallyfy without sharing credentials with your application then receive access tokens for making API requests with proper headers and security measures. </LinkTitleCard>
