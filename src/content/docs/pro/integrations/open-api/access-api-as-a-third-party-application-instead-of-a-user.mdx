---
description: Third-party applications can integrate with Tallyfy's API using the OAuth
  2.0 client credentials flow which involves obtaining client credentials from Tallyfy
  Support then authenticating to get an application-level token that enables programmatic
  user provisioning and the generation of user-specific tokens for performing API
  actions on behalf of individual users within the organization.
...
Application access tokens are valid for 7 days (`expires_in: 604800` seconds). Your application should implement automatic token refresh - simply request a new token before the current one expires.
:::
...
**Note:** No request body is required - the user's email is specified in the URL path.
...
User-specific tokens are valid for 3 months (`expires_in: 7776000` seconds). Plan your token refresh strategy accordingly.
:::
