---
about_tallyfy: true
description: Third-party applications can access Tallyfy's API on behalf of users
  through an OAuth 2.0 client credentials flow where the app first obtains an app-level
  token using its client ID and secret and then exchanges it for user-specific tokens
  to perform workflow actions as individual users within a paid organization.
