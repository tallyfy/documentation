---
about_tallyfy: true
description: Tallyfy's API lets you create standalone one-off tasks outside any process
  by sending a POST request to the tasks endpoint with a title and task type and assignees
  and deadline as required fields along with optional settings like form fields and
  webhooks and confidentiality flags.
...
public class CreateOneOffTask {
    public static void main(String[] args) {
        ...
        String jsonPayload = """
                {"title": "Java One-Off: Simple Task",
                 "task_type": "task",
                 "owners": {"users": [12345]},
                 "deadline": "2025-06-30T17:00:00Z"}""";
...
func main() {
	...
	taskData := map[string]interface{}{
		"title":     "Go One-Off Task: Send Update",
		"task_type": "task",
		"owners": map[string][]interface{}{
			"users": {1005},
		},
		"deadline": "2025-06-30T10:00:00Z",
	}
...
int main() {
    try {
        ...
        payload[U("title")] = value::string(U("C++ Task: Prepare Presentation"));
        payload[U("task_type")] = value::string(U("task"));
...
<LinkTitleCard header="<b>Processes > Launch projects</b>" href="/products/pro/integrations/open-api/code-samples/processes/launch-process-without-template/" > Tallyfy's API lets you create ad-hoc projects without a predefined template by using the `separate_task_for_each_assignee` parameter on the one-off task endpoint which spins up an empty process container and generates individual tasks for each assignee that you can then extend with additional tasks by referencing the returned `run_id`. </LinkTitleCard>
