---
about_tallyfy: true
description: Tallyfy's API lets you fetch any single task by its ID using a GET request
  and optionally include related data like form fields and process run details through
  query parameters.
...
The `form_fields`, `run`, and `step` objects only appear when you request them via the `with` parameter. If the task isn't found, you'll get a `404`. If you don't have permission, expect a `403`.
