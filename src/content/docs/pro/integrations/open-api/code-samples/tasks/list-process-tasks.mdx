---
about_tallyfy: true
description: Retrieve all tasks for a specific process run through Tallyfy's API.
  Filter by status, deadline, or owner, sort and paginate results, and include related
  data like form fields or step details.
...
-   `without_pagination` (boolean): Set to `true` if you don't want paginated results.
...
You'll get a JSON object with a `data` array of tasks and a `meta` object for pagination.
...
## Related articles
<CardGrid>
<LinkTitleCard header="<b>Tasks > List organization tasks</b>" href="/products/pro/integrations/open-api/code-samples/tasks/list-org-tasks/" > Tallyfy's API lets you retrieve and filter all tasks across an entire organization—including both process-based and one-off tasks—using a GET endpoint with extensive query parameters for status and deadline filtering and owner/guest/group assignment and sorting and pagination with code examples in six languages. </LinkTitleCard>
