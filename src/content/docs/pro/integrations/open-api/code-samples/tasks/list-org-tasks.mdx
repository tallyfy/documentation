---
about_tallyfy: true
description: Tallyfy's API endpoint `GET /organizations/{org_id}/tasks` retrieves
  all tasks across every process and one-off task in an organization with extensive
  filtering options including status and deadline and assignment and sorting and pagination
  through query parameters and returns a paginated JSON array of task objects with
  metadata.
...
<LinkTitleCard header="<b>Tags > List tags</b>" href="/products/pro/integrations/open-api/code-samples/tags/list-tags/" > Tallyfy's API lets you retrieve all tags in an organization via a GET request with optional... </LinkTitleCard>
</CardGrid>