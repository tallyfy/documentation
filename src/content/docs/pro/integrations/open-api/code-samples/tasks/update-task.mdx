---
about_tallyfy: true
description: Tallyfy's API allows you to update both process and one-off tasks via
  PUT requests — changing titles and deadlines and assignees and form field values
  — where the `owners` field fully replaces existing assignees and the `taskdata`
  object uses field IDs as keys to set values for different input types like text
  and dropdowns and checkboxes.
...
## Related articles
<CardGrid>
<LinkTitleCard header="<b>Processes > Update process</b>" href="/products/pro/integrations/open-api/code-samples/processes/update-process/" > Tallyfy's API lets you partially update a running process by sending a PUT request with only the... </LinkTitleCard>
