...
sidebar:
  order: 5
snippet: The API endpoint enables updating task properties like title summary deadline
  assignees and form field values through a PUT request while handling both process-run
  tasks and standalone one-off tasks with appropriate response validation.
title: Update task
---
