---
description: The API endpoint allows updating existing tasks by sending PUT requests
  with JSON payloads containing modified fields like title summary deadline assignees
  and form data while supporting both process-based and standalone tasks through different
  URL structures.
...
## Related articles
<CardGrid>
<LinkTitleCard header="<b>Processes > Update process</b>" href="/products/pro/integrations/open-api/code-samples/processes/update-process/" > The PUT endpoint allows modification of existing process run properties including name summary owner tags folders and form field values by sending a JSON request with only the fields to be changed. </LinkTitleCard>
<LinkTitleCard header="<b>Tasks > Complete task</b>" href="/products/pro/integrations/open-api/code-samples/tasks/complete-task/" > This API endpoint marks tasks as complete using different URLs for process tasks versus one-off tasks while supporting optional approval decisions and simultaneous form field updates through POST requests with proper authentication headers. </LinkTitleCard>
