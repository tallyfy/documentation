---
about_tallyfy: true
description: Tallyfy's API lets you soft-delete (archive) a standalone one-off task
  by sending a DELETE request to `/organizations/{org_id}/tasks/{task_id}` which hides
  it from default views while preserving all data and allowing restoration later through
  a separate restore endpoint.
