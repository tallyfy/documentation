---
about_tallyfy: true
description: Mark process tasks or standalone tasks as complete by POSTing the task ID to the right endpoint. Optionally handle approval decisions or override the completing user, with all required form fields filled beforehand.
...
> **Note:** You can't send form field values in the completion request. Update form fields through the [Update Task](/products/pro/integrations/open-api/code-samples/tasks/update-task/) endpoint before completing. The API validates that all required form fields are filled before allowing completion.
