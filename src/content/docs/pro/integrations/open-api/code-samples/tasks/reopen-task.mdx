...
## Related articles
<CardGrid>
<LinkTitleCard header="<b>Tasks > Complete task</b>" href="/products/pro/integrations/open-api/code-samples/tasks/complete-task/" > The API endpoint allows marking tasks as complete by sending a POST request with task ID and optional fields like approval status and form data while supporting both process-run tasks and standalone one-off tasks through different URL structures. </LinkTitleCard>
...
<LinkTitleCard header="<b>Tasks > Archive task</b>" href="/products/pro/integrations/open-api/code-samples/tasks/archive-task/" > A DELETE endpoint archives standalone tasks by hiding them from default views while preserving data for potential future restoration through authenticated API requests with provided code examples in multiple programming languages. </LinkTitleCard>
