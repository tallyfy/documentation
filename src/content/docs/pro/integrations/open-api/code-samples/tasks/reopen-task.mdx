---
description: This API endpoint reopens completed tasks by using a DELETE request on
  the completed-tasks endpoint which removes the completion record and restores the
  task to its previous active status.
...
<LinkTitleCard header="<b>Processes > Activate process</b>" href="/products/pro/integrations/open-api/code-samples/processes/activate-process/" > The PUT endpoint activates an archived process run by making it operational again through a simple request requiring only authorization headers and organization/run IDs with no request body needed. </LinkTitleCard>
</CardGrid>