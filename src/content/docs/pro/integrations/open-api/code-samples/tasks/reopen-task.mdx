---
about_tallyfy: true
description: Reopen a completed Tallyfy task via DELETE on the completed-tasks endpoint,
  setting its status back to in-progress for both process and one-off tasks.
