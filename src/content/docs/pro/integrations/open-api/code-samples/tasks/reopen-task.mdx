---
about_tallyfy: true
description: Tallyfy's API allows you to reopen a completed task by sending a DELETE
  request to the completed-tasks endpoint which resets the task status back to in-progress
  and clears all completion-related fields like completer ID and approval status for
  both process-based and standalone one-off tasks.
