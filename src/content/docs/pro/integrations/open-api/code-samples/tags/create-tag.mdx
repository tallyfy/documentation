---
about_tallyfy: true
description: The Tallyfy API lets you create a new tag within an organization by sending
  a POST request to `/organizations/{org_id}/tags` with a required unique title (max
  30 characters) and an optional hex color code then returns the newly created tag's
  ID and details in a 201 response.
