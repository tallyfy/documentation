---
about_tallyfy: true
description: Tallyfy's API can be accessed quickly using a personal access token found
  in Settings > Integrations > REST API which expires after 6 months and requires
  three headers (Authorization Bearer token and Accept and X-Tallyfy-Client) on every
  request but gets deleted on logout so a dedicated API user account or OAuth client
  credentials flow is recommended for stable production integrations.
