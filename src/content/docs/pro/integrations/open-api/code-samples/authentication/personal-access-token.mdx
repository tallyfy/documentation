...
sidebar:
  order: 1
snippet: The Tallyfy API authentication process requires a personal access token obtained
  from the settings page which must be included in API request headers along with
  specific Accept and X-Tallyfy-Client values for making authorized API calls.
