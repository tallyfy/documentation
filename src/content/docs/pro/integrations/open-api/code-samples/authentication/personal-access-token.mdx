---
about_tallyfy: true
description: Authenticate with Tallyfy's API using a personal access token from Settings
  > Integrations > REST API. Send it as a Bearer token with Accept and X-Tallyfy-Client
  headers on every request. Tokens expire after six months or when you log out.
