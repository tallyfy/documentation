...
The primary way to authenticate with the Tallyfy API is using your personal `access_token`. This token acts on your behalf, granting the API requests the same permissions you have within the Tallyfy application. Simple and straightforward - no OAuth dance required for basic usage.
...
Direct API access using personal tokens is typically available on paid Tallyfy plans. If you are on a trial or free plan, you might need to contact Tallyfy Support to request API access (they're pretty responsive).
:::
...
-   `X-Tallyfy-Client: APIClient` (Identifies the request is coming from a custom API client - don't forget this one or you'll get 401s)
...
Remember to replace `YOUR_PERSONAL_ACCESS_TOKEN` and `YOUR_ORGANIZATION_ID` with your actual values. (And please don't copy-paste these placeholders into production - we've all been there, but it's never fun to debug.)
