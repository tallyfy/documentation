---
description: OAuth 2.0 Client Credentials flow enables machine-to-machine authentication
  for third-party applications requiring manual setup by Tallyfy Support with enterprise-only
  access that provides both application-level tokens for system operations and user-specific
  tokens for acting as individual users with 1-hour expiration times requiring proper
  refresh logic.
...
sequenceDiagram
 ...
 participant Support as Tallyfy Support
 participant Auth as Tallyfy Auth (go.tallyfy.com/api/oauth)
 participant API as Tallyfy API (go.tallyfy.com/api)
 
 Note over App,Support: One-time Setup
 App->>Support: Request client credentials
 Support-->>App: Client ID & Client Secret
 
 Note over App,API: Application-Level Authentication
 App->>Auth: POST /oauth/token grant_type=client_credentials client_id, client_secret, scope=*
 Auth-->>App: Access token (expires in 3600s)
 
 App->>API: API Request Bearer: {app_access_token}
 API-->>App: Response data
 
 Note over App,API: Optional: User-Specific Operations
 App->>API: POST /applications/{orgID}/users/{email}/token Bearer: {app_access_token}
 API-->>App: User-specific access token
 
 App->>API: GET /organizations/{orgID}/me/tasks Bearer: {user_access_token}
 API-->>App: User's tasks data
 
 Note over App,Auth: Token Refresh Required Before Expiry
 App->>Auth: POST /oauth/token (repeat with credentials)
...
## Related articles
<CardGrid>
<LinkTitleCard header="<b>Open Api > API usage as a third-party application instead of a user</b>" href="/products/pro/integrations/open-api/access-api-as-a-third-party-application-instead-of-a-user/" > The OAuth 2.0 Client Credentials flow enables third-party applications to integrate with Tallyfy API by obtaining client credentials from support then following a secure authentication chain where applications first get app-level tokens to provision users and generate user-specific tokens allowing organizations to embed workflow functionality automate processes and manage users programmatically with proper security considerations and troubleshooting support. </LinkTitleCard>
<LinkTitleCard header="<b>Open Api > OAuth authorization flow for third-party applications</b>" href="/products/pro/integrations/open-api/oauth-authorization-flow/" > OAuth authorization flow allows third-party applications to integrate with Tallyfy by enabling users to authenticate directly with Tallyfy without sharing credentials through a standard OAuth 2.0 implicit flow that redirects users to Tallyfy's authentication page and returns access tokens for subsequent API requests. </LinkTitleCard>
