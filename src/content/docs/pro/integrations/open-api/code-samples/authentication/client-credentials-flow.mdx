...
sequenceDiagram
    ...
    participant Auth as Tallyfy Auth<br/>(go.tallyfy.com/api/oauth)
    participant API as Tallyfy API<br/>(go.tallyfy.com/api)
    ...
    App->>Auth: POST /oauth/token<br/>grant_type=client_credentials<br/>client_id, client_secret, scope=*
    Auth-->>App: Access token (expires in 3600s)
    
    App->>API: API Request<br/>Bearer: {app_access_token}
    API-->>App: Response data
    
    Note over App,API: Optional: User-Specific Operations
    App->>API: POST /applications/{orgID}/users/{email}/token<br/>Bearer: {app_access_token}
    API-->>App: User-specific access token
    
    App->>API: GET /organizations/{orgID}/me/tasks<br/>Bearer: {user_access_token}
    API-->>App: User's tasks data
    
    Note over App,Auth: Token Refresh Required Before Expiry
    App->>Auth: POST /oauth/token<br/>(repeat with credentials)
