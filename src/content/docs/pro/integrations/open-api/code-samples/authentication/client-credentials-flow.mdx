---
id: api-auth-client-credentials
sidebar:
  order: 2
title: Use the Client Credentials Flow
snippet: Learn how to authenticate third-party applications using the OAuth 2.0 Client Credentials grant type.
---
...
-   **Base URL:** `https://api.tallyfy.com` (Note: The older documentation mentioned `account.tallyfy.com`, but use `api.tallyfy.com` for consistency unless specifically told otherwise by support).
...
```bash
curl -X POST https://api.tallyfy.com/oauth/token \
