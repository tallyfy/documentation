...
description: The OAuth 2.0 Client Credentials flow enables machine-to-machine authentication
  for third-party applications to interact with the Tallyfy API through a secure process
  involving client credentials from support application-level tokens for system operations
  and optional user-specific tokens for acting as individual users within organizations.
