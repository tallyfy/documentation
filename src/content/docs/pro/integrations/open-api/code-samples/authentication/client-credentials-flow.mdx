...
description: The OAuth 2.0 Client Credentials flow enables machine-to-machine authentication
  ...
  credentials that generate application-level tokens for system operations and optional
  user-specific tokens for acting as individual users with both token types expiring
  after one hour.
