...
sidebar:
  order: 2
snippet: OAuth 2.0 Client Credentials flow enables secure machine-to-machine authentication
  for third-party applications to interact with Tallyfy API through client ID and
  secret tokens while supporting user provisioning and system-level integrations.
