...
Auth: Tallyfy Auth\n(go.tallyfy.com/api/oauth)
API: Tallyfy API\n(go.tallyfy.com/api)

# One-time Setup
App -> Support: "1. Request client credentials"
Support -> App: "2. Client ID & Client Secret" {style.stroke-dash: 3}

# Application-Level Authentication
App -> Auth: "3. POST /oauth/token\ngrant_type=client_credentials\nclient_id, client_secret, scope=*"
Auth -> App: "4. Access token\n(expires in 3600s)" {style.stroke-dash: 3}

App -> API: "5. API Request\nBearer: {app_access_token}"
API -> App: "6. Response data" {style.stroke-dash: 3}

# Optional: User-Specific Operations
App -> API: "7. POST /applications/{orgID}/users/{email}/token\nBearer: {app_access_token}"
API -> App: "8. User-specific access token" {style.stroke-dash: 3}

App -> API: "9. GET /organizations/{orgID}/me/tasks\nBearer: {user_access_token}"
API -> App: "10. User's tasks data" {style.stroke-dash: 3}

# Token Refresh Required Before Expiry
App -> Auth: "11. POST /oauth/token\n(repeat with credentials)"
Auth -> App: "12. New access token" {style.stroke-dash: 3}
```
