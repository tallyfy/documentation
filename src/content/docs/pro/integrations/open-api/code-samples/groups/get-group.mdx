---
about_tallyfy: true
description: The Tallyfy API lets you fetch details of a specific group within your
  organization by sending a GET request to `/organizations/{org_id}/groups/{group_id}`
  and it returns the group's name and description along with member IDs and guest
  emails and timestamps with an optional `with=assets` parameter to include logo file
  details.
