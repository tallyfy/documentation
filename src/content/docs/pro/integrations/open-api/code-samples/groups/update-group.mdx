---
description: This endpoint allows you to modify an existing group's properties like
  name and description as well as its membership by sending a PUT request with the
  updated fields in JSON format where providing member or guest arrays will completely
  replace the existing lists rather than incrementally updating them.
