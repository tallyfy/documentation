---
description: The POST endpoint creates a new group within an organization by accepting
  a JSON payload with required name field and optional description members and guests
  parameters while returning the created group details with assigned ID upon successful
  completion.
...
<LinkTitleCard header="<b>Tags > Create tag</b>" href="/products/pro/integrations/open-api/code-samples/tags/create-tag/" > A POST endpoint allows creation of new organizational tags by submitting JSON data with title and optional color properties through authenticated requests that return the created tag's details including its unique identifier. </LinkTitleCard>
