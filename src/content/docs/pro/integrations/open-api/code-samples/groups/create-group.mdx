---
about_tallyfy: true
description: Tallyfy's API lets you create a new group by sending a POST request to
  `/organizations/{org_id}/groups` with a unique name and description along with optional
  member user IDs and guest email addresses and returns the new group's details including
  its ID for future operations.
...
<LinkTitleCard header="<b>Groups > Get group</b>" href="/products/pro/integrations/open-api/code-samples/groups/get-group/" > Retrieve details of a specific group in your organization by its ID using a GET request with code examples in multiple languages. </LinkTitleCard>
