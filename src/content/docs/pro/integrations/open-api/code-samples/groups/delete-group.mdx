...
description: Tallyfy's DELETE endpoint at /organizations/{org_id}/groups/{group_id}
  permanently removes a group from your organization without deleting the actual members
  or guests and returns a 204 No Content status on success with code samples available
...
<LinkTitleCard header="<b>Groups > Get group</b>" href="/products/pro/integrations/open-api/code-samples/groups/get-group/" > The Tallyfy API lets you fetch details of a specific group within your organization by sending a GET request to `/organizations/[org_id]/groups/[group_id]` and it returns the group's name and description along with member IDs and guest emails and timestamps with an optional `with=assets` parameter to include logo file details. </LinkTitleCard>
