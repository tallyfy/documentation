---
about_tallyfy: true
description: Tallyfy's PUT endpoint at `/organizations/{org_id}/guests/{guest_email}`
  lets you update only the `associated_members` field for an existing guest by sending
  an array of member IDs and returns the full guest record with a 201 status on success.
