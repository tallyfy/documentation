---
about_tallyfy: true
description: Retrieve a specific guest's details by making a GET request to `/organizations/{org_id}/guests/{guest_email}`
  with a URL-encoded email. You can also include completion statistics with the `with=stats`
  query parameter.
