---
about_tallyfy: true
description: Tallyfy's API lets you retrieve detailed information about a specific
  guest user by making a GET request to `/organizations/{org_id}/guests/{guest_email}`
  with the email URL-encoded and you can optionally include completion statistics
  by passing the `with=stats` query parameter.
