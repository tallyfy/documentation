---
description: The DELETE endpoint removes a guest user from an organization by email
  address and includes code samples in multiple programming languages with proper
  URL encoding and response handling for successful deletion status codes.
...
<LinkTitleCard header="<b>Guests > Update guest</b>" href="/products/pro/integrations/open-api/code-samples/guests/update-guest/" > The PUT endpoint modifies existing guest user details in an organization by accepting JSON data with fields like name and company information while returning the updated guest record upon successful completion. </LinkTitleCard>
