...
sidebar:
  order: 1
snippet: The content explains how to upload and attach files to form fields in Tallyfy
  by first uploading the file to get an asset reference then linking it to a task
  or process through API endpoints with code examples in multiple languages.
