---
description: Attaching files to form fields via the API involves uploading the file
  content to get an asset reference then using that reference to update the corresponding
  task or process by linking the file to the specific form field.
