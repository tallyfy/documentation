...
description: Tallyfy's API lets you retrieve metadata for any uploaded file (asset)
  by sending a GET request to `/organizations/{org_id}/assets/{asset_id}` which returns
  details like the filename and upload date and the parent object the file is attached
  to along with storage and version information.
