---
about_tallyfy: true
description: Tallyfy's API lets you retrieve metadata for any uploaded file (asset)
  by calling GET on the asset endpoint with your organization and asset IDs. It returns
  details like the filename, upload date, and which process or task the file is attached
  to.
...
This gets the details of one uploaded file. Tallyfy's API calls a file an "asset". You get details like the filename, the upload date, and what the file is attached to.

## Request

Replace `{org_id}` with your organization ID. Replace `{asset_id}` with the file's asset ID.
...
You'll get `200 OK`. The file's details are in the `data` property of the JSON.
...
You'll get an error if the asset ID isn't found or you don't have permission. The status code is `404` or `403`.
