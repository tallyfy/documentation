---
description: The Tallyfy API provides endpoints to download or view files inline using
  GET requests with organization and file IDs where the response returns raw file
  content with appropriate Content-Type headers rather than JSON data.
...
- `GET /organizations/{org_id}/file/{file_id}` — View the file inline (where supported).
- `GET /organizations/{org_id}/file/{file_id}/dl` — Download the file as an attachment.

Both endpoints return the actual file content for an asset previously uploaded to Tallyfy (e.g., via a task field or kick-off field). Use the `/dl` suffix when you explicitly want a download attachment response.
...
// to use Blob creation and download triggering.
...
To display the file inline (when the client supports the MIME type), request the inline endpoint without `/dl`:
