---
description: The API provides two endpoints for retrieving file content with GET requests
  that return raw file data rather than JSON where one displays files inline and the
  other forces download as attachment with proper Content-Type and Content-Disposition
  headers for filename handling.
...
<LinkTitleCard header="<b>Files > Upload & attach file</b>" href="/products/pro/integrations/open-api/code-samples/files/upload-attach-file/" > The Tallyfy API enables file uploads to form fields through a two-step process where files are first uploaded to get an asset reference then attached to specific task or kick-off form fields using that reference. </LinkTitleCard>
</CardGrid>