...
description: 'Tallyfy''s API provides two GET endpoints for retrieving uploaded files
  by asset ID: one that returns the file inline with its original MIME type for browser
  display and another with a `/dl` suffix that forces a download with an `application/octet-stream`
  content type and attachment disposition header.'
