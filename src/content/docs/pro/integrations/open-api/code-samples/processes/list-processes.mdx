---
about_tallyfy: true
description: Tallyfy's API endpoint `GET /organizations/{org_id}/runs` retrieves a
  paginated and filterable list of active process instances (runs) in your organization
  with support for searching by name and filtering by status ownership template tags
  folders and more while returning progress details and pagination metadata for easy
  traversal of results.
