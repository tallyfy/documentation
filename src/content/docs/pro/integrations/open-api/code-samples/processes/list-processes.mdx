---
about_tallyfy: true
description: Use the GET runs endpoint to retrieve a paginated, filterable list of
  running processes in your organization. Supports optional parameters for status,
  ownership, tags, sorting, and including related data.
...
The `prerun` object on each run holds that process's kick-off form answers, keyed by each field's timeline ID. Each `prerun` value's shape depends on the field type. The per-type list lives on [Launch process](/products/pro/integrations/open-api/code-samples/processes/launch-process/).
