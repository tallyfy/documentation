---
about_tallyfy: true
description: The Tallyfy API allows you to partially update a running process by sending
  a PUT request to `/organizations/{org_id}/runs/{run_id}` with only the fields you
  want to change — such as name and summary and owner and starred status and assignees
  — but note that sending user or group arrays fully replaces all existing assignees
  of that type.
...
Send a JSON object with only the fields you want to modify. Here are the updatable fields:
...
## Related articles
<CardGrid>
<LinkTitleCard header="<b>Tasks > Update task</b>" href="/products/pro/integrations/open-api/code-samples/tasks/update-task/" > Tallyfy's API allows you to update both process and one-off tasks via PUT requests — changing... </LinkTitleCard>
<LinkTitleCard header="<b>Templates > Update template</b>" href="/products/pro/integrations/open-api/code-samples/templates/update-template/" > The Tallyfy API allows you to update an existing template by sending a PUT request to... </LinkTitleCard>
