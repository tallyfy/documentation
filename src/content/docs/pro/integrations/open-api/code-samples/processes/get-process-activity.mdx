...
description: Tallyfy's activity feed API endpoint lets you pull a full audit trail
  of every event on a process, including task completions, comments, and status changes.
  Call GET on the activity-feeds resource with filters for entity type, verb, date
  range, and actor while supporting pagination and role-based access control.
