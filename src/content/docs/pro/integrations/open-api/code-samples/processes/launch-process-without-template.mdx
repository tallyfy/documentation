...
description: Tallyfy's API lets you create ad-hoc projects without a predefined template.
  Use the `separate_task_for_each_assignee` parameter on the one-off task endpoint to
  spin up an empty process container with individual tasks per assignee, then add more
  tasks by referencing the returned `run_id`.
