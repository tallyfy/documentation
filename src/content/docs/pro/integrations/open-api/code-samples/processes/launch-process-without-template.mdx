...
<Card title="Rate limiting" icon="warning">
  Rapid bursts of write requests get throttled with HTTP 429 responses. When creating processes in bulk, add delays between requests and wait the number of seconds in the `Retry-After` header before retrying.
</Card>
