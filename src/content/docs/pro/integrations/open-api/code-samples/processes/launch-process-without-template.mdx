---
about_tallyfy: true
description: Tallyfy's API lets you create ad-hoc projects without a predefined template
  by using the `separate_task_for_each_assignee` parameter on the one-off task endpoint
  which spins up an empty process container and generates individual tasks for each
  assignee that you can then extend with additional tasks by referencing the returned
  `run_id`.
