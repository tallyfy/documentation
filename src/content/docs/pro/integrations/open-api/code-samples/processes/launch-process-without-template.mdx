---
about_tallyfy: true
description: Tallyfy's API allows you to launch an ad-hoc "empty shell" process without
  a predefined template by using the one-off task endpoint with the `separate_task_for_each_assignee`
  parameter set to true which automatically creates an empty process container and
  assigns individual tasks to each specified user or guest or group member for unified
  tracking and later expansion.
...
<LinkTitleCard header="<b>Processes > Launch process</b>" href="/products/pro/integrations/open-api/code-samples/processes/launch-process/" > Tallyfy's Launch Process API endpoint (POST to /organizations/[org_id]/runs) lets you... </LinkTitleCard>
