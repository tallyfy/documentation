---
about_tallyfy: true
description: Tallyfy's Launch Process API endpoint (POST /organizations/{org_id}/runs)
  lets you programmatically start a new process from a template by specifying the
  template ID along with optional parameters like process name and kick-off form values
  and per-step assignee or deadline overrides.
