...
sidebar:
  order: 1
snippet: The Launch Process API endpoint enables programmatic creation of process
  instances by sending a POST request with template details prerun data and task assignments
  while receiving the newly created run ID in response.
