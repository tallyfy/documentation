...
sidebar:
  order: 1
snippet: A comprehensive REST API endpoint that enables launching new process instances
  by sending a POST request with required template ID and name along with optional
  configurations for pre-filling fields and overriding task properties.
