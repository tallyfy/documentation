---
description: This endpoint uses a PUT request to restore a previously archived Tallyfy
  process instance back to its active state by removing the archived timestamp and
  updating its status through authenticated API calls with organization and run identifiers.
...
```json
{
  "data": {
    ...
    "last_updated": "..." // Reflects activation time
  }
}
```
