---
about_tallyfy: true
description: Tallyfy's API allows you to restore a previously archived process run
  by sending a PUT request to the activate endpoint which clears the `archived_at`
  field and makes the run visible again in default views while preserving its original
  status.
