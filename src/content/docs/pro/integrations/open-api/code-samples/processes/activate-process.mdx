---
about_tallyfy: true
description: Tallyfy's API lets you restore an archived process run by sending a PUT
  request to the activate endpoint which clears the `archived_at` field and brings
  the run back into default views while preserving its original status.
