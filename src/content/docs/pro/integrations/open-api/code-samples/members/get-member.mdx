---
about_tallyfy: true
description: Tallyfy's API allows admins to fetch a specific organization member's
  profile by their numeric user ID through a GET request to `/organizations/{org_id}/users/{user_id}`
  with optional related data like stats and groups included via query parameters.
