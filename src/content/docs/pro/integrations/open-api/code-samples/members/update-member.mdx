---
about_tallyfy: true
description: Tallyfy's API lets admins update a member's profile (name and timezone
  and optional fields like phone or job title) via a PUT request to `/organizations/{org_id}/users/{user_id}`
  and returns the full updated profile on success.
