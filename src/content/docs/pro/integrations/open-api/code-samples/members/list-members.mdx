---
about_tallyfy: true
description: Tallyfy's API lets you retrieve a paginated list of organization members
  via a GET request. Filter by name or email, sort results, and include related data
  like groups and stats.
...
-   `per_page` (integer): Number of results per page. Defaults to `9999` if you don't specify one.
...
You'll get a `200 OK` status code and a JSON object with a `data` array of member objects, plus a `meta` object with pagination details.
