---
about_tallyfy: true
description: Tallyfy's API allows administrators to change any organization member's
  role to admin or standard or light by sending a PUT request with the new role value
  to the member's role endpoint and receiving the updated member profile in response.
...
<LinkTitleCard header="<b>Code Samples > Managing members (users)</b>" href="/products/pro/integrations/open-api/code-samples/members/" > Tallyfy's API lets you manage organization members through `/users` endpoints to invite list... </LinkTitleCard>
