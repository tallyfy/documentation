---
about_tallyfy: true
description: Tallyfy's API lets administrators change any organization member's role
  to admin or standard or light by sending a PUT request with the new role value to
  the member's role endpoint and returning the updated member profile on success.
