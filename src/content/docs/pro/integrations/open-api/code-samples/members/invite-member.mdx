---
about_tallyfy: true
description: Invite new users to your Tallyfy organization by sending a POST request
  with their email, name, role, and a personal message. The response returns the
  invited user's profile with an invited status until they accept.
...
> Non-admin users can't assign the `admin` role. The organization must also allow non-admin users to send invites, or the request won't work.
