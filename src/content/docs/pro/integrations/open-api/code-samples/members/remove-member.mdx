...
The permanent delete endpoint requires the member to be disabled first -- you'll get an error if you skip that step. The main `DELETE /organizations/{org_id}/users/{user_id}` endpoint documented here doesn't require disabling first. It never deletes the member for good. To do that, see [disable, enable or delete member](/products/pro/integrations/open-api/code-samples/members/disable-or-delete-member/).
