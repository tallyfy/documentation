---
description: This endpoint removes a member from a Tallyfy organization using a required
  two-step process where the user must first be disabled before permanent deletion
  and optionally allows task reassignment to another member during removal.
