---
description: The endpoint removes a member from a Tallyfy organization using a two-step
  deletion process where users must first be disabled before permanent deletion with
  optional task reassignment to other members.
