...
description: 'Tallyfy''s API supports removing templates through a two-step process:
  first archiving (soft delete) via `DELETE /organizations/{org_id}/checklists/{checklist_id}`
  which hides the template but preserves data and allows restoration and then permanently
  deleting via `DELETE /organizations/{org_id}/checklists/{checklist_id}/delete` which
...
## Related articles
<CardGrid>
<LinkTitleCard header="<b>Tasks > Archive task</b>" href="/products/pro/integrations/open-api/code-samples/tasks/archive-task/" > Tallyfy's API allows you to soft-delete (archive) a standalone one-off task by sending a DELETE... </LinkTitleCard>
...
<LinkTitleCard header="<b>Tags > Delete tag</b>" href="/products/pro/integrations/open-api/code-samples/tags/archive-tag/" > Tallyfy's DELETE endpoint at /organizations/[org_id]/tags/[tag_id] permanently removes a tag and... </LinkTitleCard>
</CardGrid>