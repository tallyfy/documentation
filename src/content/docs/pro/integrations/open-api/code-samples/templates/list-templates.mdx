---
description: The List Templates API endpoint retrieves process templates (called Blueprints
  in the API) from a specified organization using a GET request to /organizations/{org_id}/checklists
...
<LinkTitleCard header="<b>Templates > Create a template</b>" href="/products/pro/integrations/open-api/code-samples/templates/create-template/" > The Tallyfy API provides a POST endpoint for creating new process templates in organizations with customizable properties like title summary permissions and workflow configuration through JSON requests. </LinkTitleCard>
</CardGrid>