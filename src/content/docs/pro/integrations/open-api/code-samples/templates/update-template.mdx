---
about_tallyfy: true
description: Use the Tallyfy API to update an existing template's properties through a PUT request. The title field is required, and array fields like users and groups fully replace existing lists.
...
When you update `users` or `groups`, the provided array *replaces* the entire existing list. It won't append new entries, so make sure you include the complete intended list.
:::
