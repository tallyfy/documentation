---
description: This endpoint enables updating existing process template properties by
  sending a PUT request with only the fields you want to modify such as title summary
  owner guidance or starred status while noting that updating user or group arrays
  will replace the entire existing list rather than adding to it.
