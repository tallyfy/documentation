---
about_tallyfy: true
description: The Tallyfy API allows you to update an existing template by sending
  a PUT request to `/organizations/{org_id}/checklists/{checklist_id}` with a required
  `title` field and optional properties like guidance and ownership while noting that
  any `users` or `groups` arrays you provide will fully replace the existing lists
...
## Related articles
<CardGrid>
<LinkTitleCard header="<b>Templates > Create a template</b>" href="/products/pro/integrations/open-api/code-samples/templates/create-template/" > Tallyfy's API allows you to create a new template (called a "checklist" in the API) by sending a... </LinkTitleCard>
