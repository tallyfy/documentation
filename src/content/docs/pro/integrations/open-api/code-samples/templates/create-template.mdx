---
about_tallyfy: true
description: Tallyfy's API allows you to create a new template (called a "checklist"
  in the API) by sending a POST request to the organization's checklists endpoint
  with required fields like title and type (procedure/form/document) and returns the
  full template object with a 201 Created status.
