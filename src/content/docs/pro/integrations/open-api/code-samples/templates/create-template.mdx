---
about_tallyfy: true
description: Tallyfy's API lets you create a new process template by sending a POST
  request to the checklists endpoint with a required title and type (procedure/form/document)
  along with optional settings like owner assignment, kick-off form fields, webhook
  URLs, and access controls. It returns the full template object with a 201 status
  code.
...
Send a `POST` request to create a new template (it's called a "checklist" in the API) inside an organization. The response returns the newly created template with a **201 Created** status.
