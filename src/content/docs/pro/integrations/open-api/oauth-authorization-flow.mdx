...
description: OAuth authorization flow enables third-party applications to authenticate
  users with Tallyfy without exposing credentials through a standard OAuth 2.0 implicit
  flow that redirects users to Tallyfy's authorization page and returns access tokens
  in URL fragments for secure API integration.
