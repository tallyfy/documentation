---
description: OAuth authorization flow allows third-party applications to integrate
  with Tallyfy by enabling users to authenticate directly with Tallyfy without sharing
  credentials through a standard OAuth 2.0 implicit flow that redirects users to Tallyfy's
  authentication page and returns access tokens for subsequent API requests.
...
sequenceDiagram
 ...
 User->>Auth: GET /oauth/authorize client_id=YOUR_CLIENT_ID redirect_uri=YOUR_REDIRECT_URL
 Auth->>User: Show login page
 User->>Auth: Enter credentials
 Auth->>Auth: Validate credentials
 Auth->>User: Redirect with token
 User->>App: YOUR_REDIRECT_URL #access_token=TOKEN &expires_in=840
 ...
 Note over App: Token expires after specified seconds
```
