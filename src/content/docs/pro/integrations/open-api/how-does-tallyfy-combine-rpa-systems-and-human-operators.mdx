...
sequenceDiagram
    autonumber
    participant RPA as RPA System<br/>(UiPath/Blue Prism)
    participant API as Tallyfy API<br/>(Orchestration)
    participant Human as Human Operator
    participant UI as Tallyfy UI<br/>(Visibility)
    
    Note over RPA,UI: Employee Onboarding Process Example
    
    API->>RPA: Trigger automated task<br/>(System access provisioning)
    RPA->>RPA: Execute bot workflow
    RPA->>API: Task completed<br/>+ Output data
    API->>UI: Update process status
    
    API->>Human: Assign manual task<br/>(Document review)
    Human->>UI: View task details
    Human->>Human: Review & make decision
    Human->>API: Complete task<br/>+ Decision data
    API->>UI: Update process status
    ...
    API->>UI: Process completed<br/>Full visibility maintained
```
