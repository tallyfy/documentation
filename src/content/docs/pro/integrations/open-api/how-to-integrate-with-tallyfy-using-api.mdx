---
about_tallyfy: true
description: Tallyfy's REST API lets you connect workflow features to external systems
  using OAuth 2.0 authentication. Covers required headers, API terminology mapping,
  token management, and multi-organization setup.
...
The Tallyfy API uses different names than what you'll see in the web app. Here's the mapping you'll need when reading API responses, building integrations, or browsing the [API reference](https://go.tallyfy.com/api/):
...
### Descriptions that reference form fields or people

The `summary`, `title`, `guidance`, `default_value` and `content` parameters accept HTML, and inside that HTML you can reference a form field's answer, a snippet, another template, or a person. Each reference type is a specific piece of markup carrying a specific identifier.

Send the wrong shape and the API still accepts it, so nothing tells you it was wrong. See [rich text references](/products/pro/integrations/open-api/rich-text-references/) for the exact markup, where to read each identifier from the API, and what happens when a reference stops resolving.
