---
description: Tallyfy provides a REST API for workflow integration that uses different
  terminology than the UI where Checklists become Blueprints and Runs become Processes
  while supporting both user-based authentication with personal tokens and application-based
  OAuth 2.0 authentication with proper token refresh handling for expired credentials.
