...
description: Tallyfy provides a REST API for integrating workflow functionality into
  external applications with two authentication methods - user-based tokens and application-based
  OAuth credentials - requiring specific headers and proper token management for accessing
  endpoints that map API objects like Checklists to UI Blueprints and Runs to Processes.
...
sequenceDiagram
 ...
 participant Auth as Tallyfy OAuth
 participant API as Tallyfy API
 
 Note over App,API: OAuth Authentication Flow
 
 alt Password Grant (User Context)
 App->>Auth: POST /oauth/token grant_type=password client_id, client_secret username, password
 Auth->>App: access_token + refresh_token expires_in: 3600
 else Client Credentials Grant (App Context)
 App->>Auth: POST /oauth/token grant_type=client_credentials client_id, client_secret
 Auth->>App: access_token (limited scope) expires_in: 3600
 end
 
 App->>API: API Request Authorization: Bearer {token} X-Tallyfy-Client: APIClient
 
 alt Token Valid
 API->>App: 200 OK + Response Data
 else Token Expired
 API->>App: 401 Unauthenticated
 App->>Auth: POST /oauth/token grant_type=refresh_token refresh_token={token}
