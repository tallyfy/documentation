...
sequenceDiagram
    ...
    alt Password Grant (User Context)
        App->>Auth: POST /oauth/token<br/>grant_type=password<br/>client_id, client_secret<br/>username, password
        Auth->>App: access_token + refresh_token<br/>expires_in: 3600
    else Client Credentials Grant (App Context)
        App->>Auth: POST /oauth/token<br/>grant_type=client_credentials<br/>client_id, client_secret
        Auth->>App: access_token (limited scope)<br/>expires_in: 3600
    end
    ...
    alt Token Valid
        ...
        App->>Auth: POST /oauth/token<br/>grant_type=refresh_token<br/>refresh_token={token}
