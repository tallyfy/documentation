---
description: This guide explains how to implement advanced Postman workflows for Tallyfy
  API testing including multi-organization environment switching and cross-org data
  comparison along with performance monitoring through response time tracking and
  endpoint analysis plus mock server configuration for simulating various API conditions
  and comprehensive CI/CD integration using Newman with GitHub Actions for automated
  testing pipelines and data-driven testing approaches using CSV and JSON files.
...
You have two options for command-line collection execution:
