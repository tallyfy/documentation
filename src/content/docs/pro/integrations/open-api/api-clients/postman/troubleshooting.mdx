---
about_tallyfy: true
description: Most Tallyfy API errors in Postman stem from a missing `X-Tallyfy-Client`
  header or wrong grant type or expired tokens and can be resolved by checking these
  along with correct URL-encoded form data for authentication and proper endpoint
  paths like `/runs/` instead of `/processes/` and `/checklists/` instead of `/templates/`
  while staying within the 600-requests-per-minute rate limit.
...
<LinkTitleCard header="<b>Postman > Collection organization best practices</b>" href="/products/pro/integrations/open-api/api-clients/postman/collection-organization/" > Organizing your Postman collections around Tallyfy's API resources with clear folder prefixes... </LinkTitleCard>
