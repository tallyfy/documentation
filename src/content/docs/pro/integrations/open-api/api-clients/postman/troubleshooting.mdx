...
## Related articles
<CardGrid>
<LinkTitleCard header="<b>Postman > Authentication setup for Postman</b>" href="/products/pro/integrations/open-api/api-clients/postman/authentication-setup/" > Setting up Tallyfy's API authentication in Postman requires configuring OAuth with password grant type since most endpoints need user context and the mandatory X-Tallyfy-Client header must be included in every request along with proper token management for the one-hour expiration period. </LinkTitleCard>
