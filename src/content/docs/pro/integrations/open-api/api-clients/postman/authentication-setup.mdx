---
description: Tallyfy's API authentication in Postman requires using the password grant
  type with proper environment variables for credentials and the mandatory X-Tallyfy-Client
  header on every request while implementing automatic token refresh before the one-hour
  expiry to maintain uninterrupted API testing access.
...
- **Password grant is required** - User context is needed for most API operations
...
1. **Missing X-Tallyfy-Client header** (most common cause)
   ```
   X-Tallyfy-Client: APIClient
   ```
