...
description: Tallyfy's API authentication in Postman requires setting up OAuth 2.0
  with a password grant type along with storing credentials as environment variables
  and configuring automatic token storage and refresh scripts while ensuring every
  request includes both a Bearer token and the mandatory X-Tallyfy-Client header.
