---
about_tallyfy: true
description: Tallyfy's API authentication in Postman requires setting up OAuth 2.0
  with the password grant type along with storing credentials as environment variables
  and configuring automatic token storage and refresh through pre-request scripts
  while ensuring every request includes both the Bearer token and the required X-Tallyfy-Client
  header.
