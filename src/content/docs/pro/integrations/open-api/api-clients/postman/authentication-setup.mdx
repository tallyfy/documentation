---
description: Tallyfy's API authentication in Postman requires OAuth configuration
  with password grant type and the mandatory X-Tallyfy-Client header for most endpoints
  that need user context rather than basic client credentials.
...
sequenceDiagram
 ...
 participant O as Tallyfy OAuth /oauth/token
 participant A as Tallyfy API /api endpoints
 
 Note over P,A: Initial Authentication Setup
 
 P->>O: POST /oauth/token grant_type=password username, password client_id, client_secret
 O->>O: Validate credentials
 O-->>P: 200 OK access_token (1hr) refresh_token expires_in: 3600
 
 P->>P: Store tokens in environment Calculate expiry time
 
 Note over P,A: API Usage with Authentication
 
 P->>A: GET /organizations/{org_id}/me Authorization: Bearer {token} X-Tallyfy-Client: APIClient
 A->>A: Validate token & header
 A-->>P: 200 OK - User details
 
 Note over P,O: Token Refresh (after ~55 min)
 
 P->>P: Check token expiry (5 min buffer)
 P->>O: POST /oauth/token grant_type=refresh_token refresh_token, client_id, client_secret
 O-->>P: New access_token New refresh_token
 P->>P: Update stored tokens
 
 Note over P,A: Common Error Scenarios
 
 P->>A: GET /api/endpoint Missing X-Tallyfy-Client header
 A-->>P: 401 Unauthenticated
 
 P->>A: GET /api/endpoint Expired token
 A-->>P: 401 Token expired
```
