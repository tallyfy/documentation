...
sequenceDiagram
    ...
    DB-->>T: Template list
    T-->>P: Available templates<br/>[{id, name, steps_count}]
    
    %% Conditional template selection
    Note over P: Evaluate conditions<br/>(amount, priority, etc.)
    alt Amount < $1000
    ...
    P->>T: POST /checklists/{template}/launch<br/>+ preruns data
    T->>T: Validate kick-off fields
    T->>DB: Create process instance
    DB-->>T: Process created
    T-->>P: 201 Created<br/>{process_id, status: "active"}
