---
description: Tallyfy's API enables launching workflow processes from templates using
  template IDs with kick-off form data where templates serve as blueprints and processes
  are running instances that can be dynamically selected based on business rules and
  launched individually or in bulk with proper rate limiting.
...
P -> T: "1. GET /organizations/{org}/checklists"
T -> DB: "2. Query templates"
DB -> T: "3. Template list" {
  style.stroke-dash: 3
}
T -> P: "4. Available templates [{id, name, steps_count}]" {
  style.stroke-dash: 3
}
...
P -> condition: "5. Check amount"
condition -> P: "6. Amount < $1000\nSelect simple_template"
condition -> P: "7. Amount < $10000\nSelect manager_approval_template"
condition -> P: "8. Amount >= $10000\nSelect executive_approval_template"

# Launch process with kick-off form
P -> T: "9. POST /checklists/{template}/launch + preruns data"
T -> T: "10. Validate kick-off fields"
T -> DB: "11. Create process instance"
DB -> T: "12. Process created" {
  style.stroke-dash: 3
}
T -> P: "13. 201 Created {process_id, status: 'active'}" {
  style.stroke-dash: 3
}

# Bulk launch pattern
bulk_loop: "For each employee" {
  shape: hexagon
  style.fill: "#e8f4f8"
}
P -> bulk_loop: "14. Start bulk launch"
bulk_loop -> P: "15. Wait 200ms (rate limit)"
P -> T: "16. POST /checklists/{template}/launch"
T -> P: "17. Process launched" {
  style.stroke-dash: 3
}

# Process monitoring
P -> T: "18. GET /runs?status=active"
T -> P: "19. Active processes list" {
  style.stroke-dash: 3
}

P -> T: "20. PUT /runs/{process}/archive"
T -> DB: "21. Update status to archived"
DB -> T: "22. Status updated" {
  style.stroke-dash: 3
}
T -> P: "23. Process archived" {
  style.stroke-dash: 3
}
```
