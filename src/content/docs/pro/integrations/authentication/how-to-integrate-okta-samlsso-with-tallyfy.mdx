...
sequenceDiagram
 ...
 Note over A,S: SSO Setup Process
 
 A->>O: Create SAML app Configure SP values
 O-->>A: Provide IdP metadata (SSO URL, Issuer, X.509)
 A->>S: Send IdP configuration
 S->>T: Configure SAML settings
 S-->>A: Confirm setup complete
 A->>T: Enable SAML toggle
 
 Note over U,T: User Authentication Flow
 
 U->>O: Access Tallyfy SSO URL
 O->>U: Show Okta login
 U->>O: Enter credentials
 O->>O: Authenticate user
 O->>T: SAML assertion (email, FirstName, LastName)
 
 alt Existing user
 T->>T: Match email, login user
 else New user
 T->>T: Create account Set name from attributes
 end
