...
sequenceDiagram
    ...
    A->>O: Create SAML app<br/>Configure SP values
    O-->>A: Provide IdP metadata<br/>(SSO URL, Issuer, X.509)
    ...
    O->>T: SAML assertion<br/>(email, FirstName, LastName)
    
    alt Existing user
        T->>T: Match email, login user
    else New user
        T->>T: Create account<br/>Set name from attributes
    end
