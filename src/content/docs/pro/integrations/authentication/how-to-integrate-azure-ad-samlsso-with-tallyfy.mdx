---
description: This comprehensive guide demonstrates how to integrate Microsoft Azure
  Active Directory with Tallyfy for seamless single sign-on authentication by creating
  an Azure AD enterprise application configuring SAML settings mapping user attributes
  and enabling automatic user provisioning through coordinated setup between administrators
  and Tallyfy Support.
...
sequenceDiagram
 autonumber
 participant Admin as Azure Admin
 participant Azure as Azure AD
 participant Support as Tallyfy Support
 participant Tallyfy as Tallyfy Platform
 participant User as End User
 
 Note over Admin,Support: SSO Setup Process
 
 Admin->>Support: Submit support ticket for SSO
 Support->>Admin: Provide Tallyfy SAML values (SP ACS URL, SP Entity ID)
 
 Admin->>Azure: Create Enterprise Application
 Admin->>Azure: Configure SAML settings with Tallyfy values
 Admin->>Azure: Map user attributes (FirstName, LastName, Email)
 
 Azure->>Admin: Provide Azure SAML info (Login URL, Azure ID, Certificate)
 Admin->>Support: Send Azure SAML configuration
 
 Support->>Tallyfy: Configure SAML settings
 Support->>Admin: Confirm setup complete
 Admin->>Tallyfy: Enable SAML toggle
 
 Note over User,Tallyfy: User Authentication Flow
 
 User->>Tallyfy: Access SSO URL
 Tallyfy->>Azure: Redirect to Azure AD
 User->>Azure: Enter credentials
 Azure->>Azure: Authenticate user
 Azure->>Tallyfy: SAML assertion
 Tallyfy->>Tallyfy: Provision/authenticate user
 Tallyfy->>User: Grant access
```
