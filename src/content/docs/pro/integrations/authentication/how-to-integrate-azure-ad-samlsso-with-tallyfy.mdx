...
description: Tallyfy integrates with Microsoft Entra ID (formerly Azure Active Directory)
  for self-serve SAML single sign-on. An admin creates an enterprise app in Entra
  ID, then pastes the metadata into the SSO wizard under Settings to enable automatic
  login. Entra provisioning adds SCIM directory sync.
...
<LinkTitleCard header="<b>Authentication > Integrate JumpCloud SSO</b>" href="/products/pro/integrations/authentication/how-to-integrate-jumpcloud-samlsso-with-tallyfy/" > Set up SAML-based SSO between JumpCloud and Tallyfy yourself by creating a custom SAML app, then... </LinkTitleCard>
</CardGrid>