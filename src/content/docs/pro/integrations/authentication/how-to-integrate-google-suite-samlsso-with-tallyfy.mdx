...
sequenceDiagram
 ...
 Note over U,G: SAML Authentication Flow
 
 U->>T: Access Tallyfy SSO URL
 T->>G: Redirect to Google SSO (with SAML Request)
 G->>U: Show Google login page
 U->>G: Enter Google credentials
 G->>G: Validate credentials
 
 alt New User
 G->>T: SAML Response with attributes (email, firstName, lastName)
 T->>T: Auto-provision new user account
 T->>T: Create session
 T->>U: Grant access to Tallyfy
 else Existing User 
 G->>T: SAML Response with attributes
 T->>T: Match existing user by email
 T->>T: Create session
 T->>U: Grant access to Tallyfy
 end
 
 Note over U,T: User now authenticated via SSO
```
