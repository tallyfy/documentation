---
about_tallyfy: true
description: Tallyfy integrates with Google Workspace through SAML-based Single Sign-On
  by creating a custom SAML app in the Google Admin console and exchanging configuration
  details (SSO URL and Entity ID and certificate) with Tallyfy Support so users authenticate
  with Google credentials and new accounts are automatically provisioned on first
  login.
