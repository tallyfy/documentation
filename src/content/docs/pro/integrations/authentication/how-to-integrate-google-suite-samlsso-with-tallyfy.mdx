---
about_tallyfy: true
description: Tallyfy integrates with Google Workspace through SAML-based SSO by creating
  a custom SAML app in the Google Admin console and exchanging configuration details
  (SSO URL and Entity ID and certificate) with Tallyfy Support so users can authenticate
  with their Google credentials and get automatically provisioned on first login without
  needing separate passwords.
