...
User -> Tallyfy: 1. Open your org's SSO login link
Tallyfy -> IdP: 2. Redirect to your identity provider
User -> IdP: 3. Sign in with corporate credentials and MFA
IdP -> Tallyfy: 4. Send SAML assertion
Tallyfy -> Tallyfy: 5. Match existing user, or auto-create from a verified domain
Tallyfy -> User: 6. Grant access
```

Step 5 is where auto-join matters. If the person already has a Tallyfy account, they're matched on email and logged straight in. If they don't, and their email is on a domain you've verified, Tallyfy creates the account from the email, first name, and last name in the assertion. Without a verified domain, an unknown user is turned away rather than auto-created.
