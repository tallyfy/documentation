...
description: Tallyfy offers free, self-serve SSO and SCIM on any paid plan with support
  for Microsoft Entra ID, Google Workspace, Okta, OneLogin, JumpCloud, and any SAML 2.0
  provider. Org admins configure SSO themselves under Settings, verify a domain by DNS
  TXT, sync users with SCIM, and can enforce SSO-only login with an admin break-glass.
...
<LinkTitleCard header="<b>Mcp Server > Using SSO with MCP servers</b>" href="/products/pro/integrations/mcp-server/sso-authentication/" > SSO integration with MCP servers removes repeated authentication across AI tools. Users log in... </LinkTitleCard>
