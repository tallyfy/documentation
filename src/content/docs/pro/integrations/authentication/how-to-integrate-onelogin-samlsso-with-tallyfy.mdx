---
description: OneLogin SAML/SSO Integration enables teams to access Tallyfy using their
  existing OneLogin credentials through a comprehensive setup process involving SAML
  application creation configuration exchange between systems and automatic user provisioning
  that takes approximately 30 minutes with coordination from Tallyfy Support.
...
sequenceDiagram
 ...
 participant TSupport as Tallyfy Support
 participant Tallyfy
 participant User
 
 Note over Admin,Tallyfy: Setup Phase
 
 Admin->>OneLogin: Create SAML Test Connector app
 Admin->>OneLogin: Name it "Tallyfy"
 
 Admin->>TSupport: Request SAML configuration values
 TSupport->>Admin: Provide SP ACS URL, SP Entity ID
 
 Admin->>OneLogin: Configure with Tallyfy values - ACS URL - Entity ID - Parameters (Email, FirstName, LastName)
 
 Admin->>OneLogin: Assign users/roles to app
 
 OneLogin->>Admin: Provide SAML metadata - SAML 2.0 Endpoint - Issuer URL - X.509 Certificate
 
 Admin->>TSupport: Send OneLogin metadata
 TSupport->>Tallyfy: Configure SAML settings
 TSupport->>Admin: Confirm configuration complete
 
 Admin->>Tallyfy: Enable SAML toggle
 Tallyfy->>Admin: Provide SSO login URL
 
 Note over User,Tallyfy: Runtime Authentication Flow
 
 User->>Tallyfy: Access via SSO URL
 Tallyfy->>OneLogin: Redirect for authentication
 User->>OneLogin: Enter credentials
 OneLogin->>OneLogin: Validate user
 OneLogin->>Tallyfy: SAML assertion with user attributes
 
 alt Existing Tallyfy user
 Tallyfy->>User: Grant immediate access
 else New user
 Tallyfy->>Tallyfy: Auto-provision account
 Tallyfy->>User: Grant access to new account
 end
```
