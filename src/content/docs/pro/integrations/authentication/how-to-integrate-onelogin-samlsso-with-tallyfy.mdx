---
description: OneLogin SAML/SSO integration with Tallyfy enables team members to authenticate
  using their OneLogin credentials through a 30-minute collaborative setup process
  that involves creating a SAML custom connector application in OneLogin and configuring
  attribute mappings and exchanging metadata with Tallyfy Support to enable automatic
  authentication and user provisioning for new accounts on first login.
...
<Steps>
1. Search for "SAML Custom Connector" (or "SAML Test Connector" in older OneLogin interfaces)
