...
description: Tallyfy webhook payloads deliver JSON data in two formats — process-level
  payloads containing full process state and kick-off form data when processes launch
  or complete and task-level payloads containing the completed task details along
  with all accumulated form fields and a next_task reference pointing to the first
  visible incomplete task plus guest-links that provide direct access URLs for external
  collaborators.
