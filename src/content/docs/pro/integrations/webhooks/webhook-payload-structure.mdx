...
description: Tallyfy webhook payloads deliver JSON data in two formats — process-level
  payloads containing full process state and kick-off form data when a process launches
  or completes and task-level payloads containing the completed task details along
  with all collected form fields and a reference to the next incomplete task plus
  guest access links for external collaborators.
