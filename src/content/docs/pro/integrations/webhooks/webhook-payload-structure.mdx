---
description: Tallyfy webhooks deliver JSON payloads with different structures
...
When Tallyfy sends a webhook to your endpoint, it delivers a JSON payload containing complete data about the event. The exact structure varies based on webhook type - **process-level webhooks** (fired when a process launches or completes) versus **task-level webhooks** (fired when a specific task completes).
