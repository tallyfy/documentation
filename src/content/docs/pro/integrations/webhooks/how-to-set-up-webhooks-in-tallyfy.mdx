...
sequenceDiagram
 ...
 participant M as Middleware (Zapier/Make/n8n)
 participant E as External System (Slack/CRM/etc)
 
 Note over P,E: Webhook Event Flow
 
 P->>T: Complete step/Launch process
 T->>T: Check webhook config
 
 alt Template-level webhook
 Note over T: Fires on ANY step or launch
 T->>Q: Queue template webhook with ALL process data
 else Step-level webhook 
 Note over T: Fires on SPECIFIC step only
 T->>Q: Queue step webhook with step data
 end
 
 Q->>W: Process webhook event
 
 loop Max 3 attempts
 W->>M: POST webhook_url X-Tallyfy-OrgID: [org_id] JSON payload
 Note over W,M: Timeout: 30 seconds
 
 alt Success (2xx)
 M-->>W: Success response
 W->>T: Mark delivered
 M->>M: Validate X-Tallyfy-OrgID
 M->>E: Trigger action (Send Slack message)
 E-->>M: Action confirmed
 else Failure
 M-->>W: Error/Timeout
 W->>W: Exponential backoff
 Note over W: Retry: 1min, 5min, 15min
 end
 end
 
 alt All attempts failed
 W->>T: Mark failed
 Note over T: Log failure for debugging
 end
```
