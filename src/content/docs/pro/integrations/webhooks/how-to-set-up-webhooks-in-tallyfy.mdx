...
description: Tallyfy webhooks automatically send JSON data to external systems when
  workflow events occur with template-level webhooks firing on any step completion
  and step-level webhooks triggering only for specific steps while supporting retry
  mechanisms and security validation through organization ID headers.
