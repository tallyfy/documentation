...
sequenceDiagram
    ...
    participant M as Middleware<br/>(Zapier/Make/n8n)
    participant E as External System<br/>(Slack/CRM/etc)
    ...
    alt Template-level webhook
        ...
        T->>Q: Queue template webhook<br/>with ALL process data
    else Step-level webhook  
        Note over T: Fires on SPECIFIC step only
        T->>Q: Queue step webhook<br/>with step data
    end
    
    Q->>W: Process webhook event
    
    loop Max 3 attempts
        W->>M: POST webhook_url<br/>X-Tallyfy-OrgID: [org_id]<br/>JSON payload
        ...
        alt Success (2xx)
            ...
            M->>E: Trigger action<br/>(Send Slack message)
