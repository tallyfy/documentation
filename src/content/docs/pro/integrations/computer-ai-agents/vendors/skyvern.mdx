...
description: Skyvern automates browser workflows using LLMs and computer vision with
  ...
  can integrate with Tallyfy to handle mundane web-based tasks described in plain
  English while returning structured JSON data for seamless process automation.
...
```mermaid
flowchart TD
    T["📋 Tallyfy Task<br/>URL + Instructions"] --> W["🔄 Webhook"]
    W --> S["🤖 Skyvern API"]
    
    S --> A["Analyze Task"]
    A --> P["🎯 Planner Agent<br/>Sets Goals"]
    P --> AC["🎭 Actor Agent<br/>Executes Actions"]
    AC --> V["✅ Validator Agent<br/>Checks Success"]
    
    V -->|Success| D["📊 Data Extraction"]
    V -->|Retry| P
    
    D --> J["📄 JSON Results"]
    J --> U["📊 Update Tallyfy<br/>Form Fields"]
    U --> N["✅ Next Task"]
```
