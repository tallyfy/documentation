...
```mermaid
flowchart TB
    Start([User Task Request]) --> Planner[<b>Planner Agent</b><br/>Maintains working memory<br/>Sets goals & sub-goals]
    
    Planner -->|Goal Assignment| Actor[<b>Actor Agent</b><br/>Executes browser actions<br/>Reports completion status]
    
    Actor -->|Action Results| Validator[<b>Validator Agent</b><br/>Assesses goal success<br/>Quality control checks]
    
    Validator -->|Feedback Loop| Planner
    Validator -->|Error Correction| Actor
    
    Actor -->|Status Updates| Planner
    
    Validator -->|Success| Complete([Task Complete<br/>Returns data])
    Validator -->|Failed| Retry{Retry<br/>Logic}
    
    Retry -->|New Approach| Planner
    Retry -->|Max Attempts| Failed([Task Failed<br/>Error report])
    
    subgraph Specialized Sub-Agents
        Elements[Element Agent<br/>Parse HTML]
        Nav[Navigation Agent<br/>Plan sequences]
        Data[Data Agent<br/>Extract JSON/CSV]
        Auth[Auth Agents<br/>2FA & Passwords]
    end
    
    Actor -.->|Invokes| Elements
    Actor -.->|Invokes| Nav
    Actor -.->|Invokes| Data
    Actor -.->|Invokes| Auth
    
    style Planner fill:#e1f5e1
    style Actor fill:#e1e5f5
    style Validator fill:#f5e1e1
