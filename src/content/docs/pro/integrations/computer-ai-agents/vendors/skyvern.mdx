...
description: Skyvern automates browser workflows using LLMs and computer vision with
  an open-source architecture that achieves 85.8% accuracy on web automation benchmarks
  and can integrate with Tallyfy to handle complex browser-based tasks through natural
  language instructions while maintaining transparency and scalability for enterprise
  workflows.
...
```mermaid
flowchart TB
    Request[Task Request] --> Planner["`**<b>Planner</b>**
Sets goals`"]
    
    Planner --> Actor["`**<b>Actor</b>**
Executes actions`"]
    
    Actor --> Validator["`**<b>Validator</b>**
Checks results`"]
