...
description: Skyvern automates browser workflows using LLMs and computer vision with
  an open-source AGPL-3.0 license achieving 85.8% accuracy on the WebVoyager benchmark
  through its three-agent architecture of Planner Actor and Validator agents that
  can handle complex web tasks including CAPTCHAs 2FA and dynamic websites while integrating
  with Tallyfy to provide structured JSON data output for process automation.
...
### How Skyvern Integrates with Tallyfy

This diagram illustrates how Tallyfy sends web automation tasks to Skyvern's AI agents, which use computer vision and LLMs to extract data and return structured results.
...
**What to notice:**
- **Three-agent architecture** - Planner, Actor, and Validator agents work together to achieve 85.8% accuracy on web tasks
- **Self-correcting system** - The Validator agent can trigger retries with new approaches when tasks fail
- **Structured data output** - Results come back as clean JSON that maps directly to Tallyfy form fields
...
Planner: "Planner"
Validator: "Validator"
Request: "Task Request" -> Planner: ""<b>Planner</b> Sets goals""
Planner -> Actor: ""<b>Actor</b> Executes actions""
Actor -> Validator: ""<b>Validator</b> Checks results""
...
W: "W"
autonumber
T as Tallyfy Process
W as Webhook/Middleware
S as Skyvern API
P as Planner Agent
A as Actor Agent
D as Data Agent
B as Browser
T -> W: Task starts with form data (URL, product name, schema)
W -> S: POST /v1/tasks {"url", "prompt", "data_schema"}
S -> S: Create task ID
S- -> W: {"task_id": "abc-123", "status": "queued"}
W- -> T: Task initiated
# Note over S,B: Async Processing Begins
S -> P: Analyze task requirements
P -> P: Break into sub-goals: 1. Navigate to page 2. Find product 3. Extract data
P -> A: Execute: Navigate to URL
A -> B: Browser automation (Playwright/Selenium)
B- -> A: Page loaded
...
# end block
D -> S: Task complete {"product_name": "MegaWidget Advanced", "price": "$99.99", "stock_status": "In Stock"}
T -> W: Check status
W -> S: GET /v1/tasks/abc-123
S- -> W: {"status": "completed", "data": {...}}
W- -> T: JSON result
T -> T: Update form fields: • Competitor Price = $99.99 • Stock Status = In Stock
...
## Related articles
<CardGrid>
<LinkTitleCard header="<b>Integrations > Computer AI agents</b>" href="/products/pro/integrations/computer-ai-agents/" > Computer AI Agents are software programs that can see your screen understand interfaces and take automated actions while Tallyfy provides the control framework with step-by-step instructions guardrails and tracking to orchestrate these agents within complete business processes ensuring transparency accountability and continuous improvement. </LinkTitleCard>
...
<LinkTitleCard header="<b>Vendors > Twin.so AI agents</b>" href="/products/pro/integrations/computer-ai-agents/vendors/twin/" > Twin.so AI agents automate complex web operations through browser interactions and can integrate with Tallyfy to handle repetitive web-based tasks using enterprise-grade infrastructure that already serves 500000 European SMBs through partnerships with Qonto and OpenAI. </LinkTitleCard>
