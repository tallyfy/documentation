---
about_tallyfy: false
description: Claude Computer Use lets an AI agent control a screen through screenshots
  and mouse and keyboard actions in a sandboxed Docker environment. Tallyfy sends
  the task and captures the results, so you can automate repetitive desktop and web
  UI work like form filling and legacy data extraction, with people kept in the loop.
...
**Performance benchmarks (OSWorld[^3]):**
...
**What to notice:**
- Tallyfy triggers your intermediary app via webhook[^1] with task data.
...
- **Resolution**: it may struggle above 1024x768 or 1280x800 due to image scaling.
...
- There's a risk of prompt injection from webpages or images.
- There's potential for misuse if it isn't properly isolated.
...
- Code execution risks if it isn't properly sandboxed.
- Information theft if it's given access to sensitive data.
...
**Disadvantages:**
- Slower than traditional RPA[^2] for simple tasks
...
[^1]: Webhooks let Tallyfy and your app message each other automatically the moment something happens.
[^2]: Robotic process automation - older bots that follow fixed scripts and break when a screen changes.
[^3]: OSWorld is a standard test of how well an AI agent completes real tasks across desktop and web apps.
...
<LinkTitleCard header="<b>Computer Ai Agents > Local computer use agents</b>" href="/products/pro/integrations/computer-ai-agents/local-computer-use-agents/" > Local computer use agents run AI automation on your own hardware, so data stays private and... </LinkTitleCard>
</CardGrid>