...
<LinkTitleCard header="<b>Using Tallyfy MCP Server with Claude (Text Chat)</b>" href="/products/pro/integrations/mcp-server/claude-anthropic/" > Connect Tallyfy's MCP Server with Claude Desktop to enable natural language workflow management through text-based chat and API integration for data queries analysis and automation without visual UI interaction. </LinkTitleCard>
