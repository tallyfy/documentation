---
about_tallyfy: false
description: Claude Computer Use is Anthropic's beta capability that lets Claude visually
  perceive and control desktop environments through screenshot-analyze-act loops and
  when paired with Tallyfy it can automate repetitive UI tasks like form filling and
  data extraction from legacy systems by receiving structured task instructions via
  webhook and executing them in isolated Docker sandboxes while returning results
  back to Tallyfy for tracking.
