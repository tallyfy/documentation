...
```mermaid
flowchart TD
    Start([Agent Starts]) --> Perceive["`**1. Perceive**
Capture screen state`"]
    Perceive --> Reason["`**2. Reason**
Process context & plan`"]
    Reason --> Act["`**3. Act**
Execute action`"]
    Act --> Observe["`**4. Observe**
Capture result`"]
    Observe --> Check{Goal<br/>Achieved?}
    Check -->|"`No`"| Iterate["`**5. Iterate**
2-8 sec cycle`"]
    Iterate --> Perceive
    Check -->|"`Yes`"| Complete([Task Complete])
    
    %% Component annotations
    Perceive -.->|Uses| VLM["`**Vision-Language**
Model`"]
    Act -.->|Uses| Engine["`**Action**
Engine`"]
    Reason -.->|Uses| Orchestrator["`**Orchestration**
Framework`"]
