...
description: Tallyfy leads the revolution in local Computer Use Agents that run entirely
  on your hardware without sending sensitive data to cloud services while eliminating
  API costs and network latency through cutting-edge AI models like DeepSeek-R1 and
  Gemma 3n that process screenshots and execute computer tasks locally with complete
  privacy and zero ongoing token expenses.
...
```mermaid
flowchart TD
    subgraph Inputs[Tallyfy Task Inputs]
        I1[Step Instructions]
        I2[Input Data]
        I3[Security Rules]
    end
    
    subgraph LocalAgent[Local AI Agent]
        L1[Load Local Model]
        L2[Process Locally]
        L3[No Cloud Calls]
        L4[Execute Actions]
    ...
    subgraph Outputs[Tallyfy Results]
        O1[Action Results]
...
```mermaid
flowchart TD
 subgraph Tallyfy["Tallyfy Platform"] Task["Task with"]
 end
