---
description: Tallyfy revolutionizes business automation by developing local Computer
  Use Agents that run entirely on your hardware providing complete privacy zero latency
  no token costs and offline operation while solving every limitation of cloud-based
  AI agents through advanced multimodal models like Gemma 3n DeepSeek-R1 and Qwen3
  that deliver production-ready capabilities for enterprise workflow automation.
...
```mermaid
flowchart TD
    Start([Agent Starts]) --> Perceive[1. Perceive<br/>Capture screen state]
    Perceive --> Reason[2. Reason<br/>Process context & plan]
    Reason --> Act[3. Act<br/>Execute action]
    Act --> Observe[4. Observe<br/>Capture result]
    Observe --> Check{Goal<br/>Achieved?}
    Check -->|No| Iterate[5. Iterate<br/>2-8 sec cycle]
    Iterate --> Perceive
    Check -->|Yes| Complete([Task Complete])
    
    %% Component annotations
    Perceive -.->|Uses| VLM[Vision-Language<br/>Model]
    Act -.->|Uses| Engine[Action<br/>Engine]
    Reason -.->|Uses| Orchestrator[Orchestration<br/>Framework]
    
    click Perceive "/products/pro/integrations/computer-ai-agents/#screen-capture" "Learn about screen capture"
    click Act "/products/pro/integrations/computer-ai-agents/#action-execution" "Action execution details"
    click VLM "#core-architecture-components" "Vision-Language Model details"
```
...
```mermaid
flowchart TD
    subgraph Tallyfy[Tallyfy Platform]
        Task[Task with<br/>Instructions]
        Forms[Form Fields<br/>& Data]
        Process[Process<br/>Workflow]
    end
    
    subgraph LocalAgent[Local Computer Use Agent]
        Receive[Receive<br/>Instructions]
        Execute[Execute<br/>Actions]
        Monitor[Monitor<br/>Progress]
    end
    
    subgraph Outputs[Agent Outputs]
        Data[Extracted<br/>Data]
        Status[Completion<br/>Status]
        Logs[Audit<br/>Logs]
    end
    ...
    click Task "/products/pro/tracking-and-tasks/tasks/" "Learn about Tallyfy tasks"
