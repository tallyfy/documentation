...
description: Google Drive lacks a native file request feature so collecting files
  ...
  or File Request Pro that let anyone upload without a Google account or using Google
  Forms (which still requires sign-in) or shared folders or custom Apps Script endpoints.
