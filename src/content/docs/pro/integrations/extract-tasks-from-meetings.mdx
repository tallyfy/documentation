...
description: Tallyfy is developing an AI-powered feature that will automatically extract
  actionable tasks from meeting recordings across platforms like Zoom and Teams while
  using advanced conversation tracking to identify task owners deadlines and context
  before presenting them in a Draft state for human review and conversion to active
  tasks.
