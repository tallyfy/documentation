---
description: Tallyfy is developing an AI-powered feature that will automatically extract
  actionable tasks from meeting recordings by transcribing conversations identifying
  task assignments with proper context and deadlines then organizing them into a Draft
  state for human review before converting to trackable tasks across all major meeting
  platforms.
...
```mermaid
flowchart TD
    Start["Meeting"] --> Transcribe["Transcribe"]
    
    Transcribe --> Extract["Extract"]
    
    Extract --> Draft["Draft"]
    
    Draft --> Review{"Review"}
    
    Review -->|Approve| Convert["Open"]
    Review -->|Edit| Edit["Edit"]
    Review -->|Reject| Delete["Delete"]
    
    Edit --> Convert
    Convert --> Track["Active"]
```
