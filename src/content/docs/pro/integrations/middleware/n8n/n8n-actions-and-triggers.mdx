...
The expanded operation set (96 operations) and the new Tallyfy Trigger node are published on npm in version 1.1.0, so `npm install n8n-nodes-tallyfy` gives you the full set today. One fix is still on the way in the next release (1.1.1): correct encoding of choice-field values (dropdown, multiselect, and radio) when you pass them into a kickoff form on launch.
:::
