---
description: Connecting n8n to Tallyfy requires setting up API credentials and configuring
  either the dedicated Tallyfy node or HTTP Request nodes to enable workflow automation
  between the platforms with support for real-time webhooks and extensive operations
  across 10 resources.
...
flowchart TD
 ...
 Method -->|"Quick"| Node[Tallyfy Node]
 Method -->|"Custom"| HTTP[HTTP Request]
 
 Node --> Auth["Configure API Token"]
 HTTP --> Auth
 Auth --> Test["Test Connection"]
 Test -->|"Success"| Ready["Ready to Build"]
 Test -->|"Failed"| Debug[Check Token]
 Debug --> Auth
 
 click Node "/products/pro/integrations/middleware/n8n/index/" "n8n integration overview"
 click Ready "/products/pro/integrations/open-api/index/" "Tallyfy API documentation"
```
