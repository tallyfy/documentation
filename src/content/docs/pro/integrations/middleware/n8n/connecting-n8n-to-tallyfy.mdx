---
description: n8n can be connected to Tallyfy in about 5 minutes using either a dedicated
  community-built connector for instant setup or HTTP Request nodes for maximum flexibility
  with both methods supporting Tallyfy's Open API and seamless authentication while
  providing access to 60+ operations across 10 resources including real-time webhook
...
webhook_check: {
  T -> Q: 3. Queue webhook event |Webhook configured|
  # Event queued for delivery
  T -> T: Skip webhook |No webhook|
}

Q -> W: 4. Process webhook

retry_loop: {
  W -> n8n: 5. POST {webhook_url}
  # Timeout: 30 seconds
  
  success_path: {
    n8n -> W: 6. Success response |Success (2xx)|
    W -> T: 7. Mark delivered
    # Log success
  }
  
  failure_path: {
    n8n -> W: Error or timeout |Failure|
