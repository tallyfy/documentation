---
about_tallyfy: false
description: n8n connects to Tallyfy in about five minutes using either a community-built
  Tallyfy node or HTTP Request nodes. It supports 60+ operations across blueprints,
  processes, tasks, comments, users, and groups.
...
4. Find your Organization ID in your Tallyfy URL after you sign in - it's the identifier right after `https://go.tallyfy.com/` in your browser's address bar.
...
Treat your API token like a password. Don't share it publicly or commit it to version control. In n8n, always use the Credentials feature to store it securely.
:::
...
5. You'll see your user details, confirming authentication works.
...
3. Configure **Kickoff Field Values**. This is a repeating collection, not a JSON box: click **Add Kickoff Value** once per field, then fill in the two inputs.
   ...
   Each **Field** is resolved against the template's kickoff fields, and the **Value** is encoded for you according to that field's type. An entry that matches no field **fails the execution** rather than being dropped quietly, so a typo shows up straight away instead of launching a process with a value missing. Use the **Get Kickoff Fields** operation to list a template's fields.
...
**Tips:**
- Each **Field** is matched by ID, alias or exact label, so you do not have to reproduce the label character for character
