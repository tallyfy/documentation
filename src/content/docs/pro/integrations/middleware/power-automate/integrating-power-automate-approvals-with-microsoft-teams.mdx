---
description: Integrating Power Automate approvals with Microsoft Teams streamlines
  Tallyfy-related approval processes by centralizing notifications reducing context
  switching and enabling faster responses through interactive Adaptive Cards that
  allow approvers to make decisions directly within Teams without switching applications.
...
Teams as Microsoft Teams
U as Approver
# Note over T,U: Budget Approval Request Flow
T -> PA: Trigger flow (budget request)
PA -> PA: Start and wait for approval
PA -> Teams: Post Adaptive Card to approver
Teams -> U: Display approval request
# Note over PA: Flow paused waiting for response
U -> Teams: Click Approve/Reject
Teams -> PA: Return response
PA -> PA: Get Outcome and Comments
PA -> T: Update task/process with decision
# Note over T: Process continues based on approval
```
