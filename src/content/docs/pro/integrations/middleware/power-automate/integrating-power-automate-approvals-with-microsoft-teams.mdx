...
sequenceDiagram
    ...
    T->>PA: Trigger flow<br/>(budget request)
    PA->>PA: Start and wait<br/>for approval
    PA->>Teams: Post Adaptive Card<br/>to approver
    Teams->>U: Display approval<br/>request
    
    Note over PA: Flow paused<br/>waiting for response
    
    U->>Teams: Click Approve/Reject
    Teams->>PA: Return response
    PA->>PA: Get Outcome<br/>and Comments
    PA->>T: Update task/process<br/>with decision
    
    Note over T: Process continues<br/>based on approval
```
