...
E as Email Sender
O as Outlook Inbox
PA as Power Automate
T as Tallyfy API
TP as Tallyfy Process
# Note over E,TP: Automated Email-to-Task Flow
E -> O: Sends email with "New Sales Lead Received"
O -> O: Email arrives in Inbox
O -> PA: Trigger: New email detected
PA -> PA: Check subject filter
# alt: Subject matches filter
PA -> PA: Extract email data (Subject, Body, Sender)
PA -> T: POST Create Task Auth: API Key/Secret
# Note right of PA: Blueprint ID: Lead Follow-Up Task Name: Email Subject Description: Email Body Deadline: +1 day
T -> T: Validate request
T -> TP: Create task in process
TP- -> T: Task created (ID: xxx)
T- -> PA: 200 OK - Task created
PA -> PA: Log success
else Subject doesn't match
PA -> PA: Skip (no action)
# end block
# Note over PA,TP: Task now trackable in Tallyfy
```
