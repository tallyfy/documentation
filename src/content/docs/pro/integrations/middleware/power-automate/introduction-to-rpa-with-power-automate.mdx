---
description: Power Automate Desktop enables RPA automation to bridge Tallyfy with
  legacy desktop applications lacking APIs by using software bots that interact with
  user interfaces to automate repetitive tasks and data entry between modern cloud
  workflows and older systems.
...
sequenceDiagram
 ...
 participant User
 participant Tallyfy
 participant CloudFlow as Power Automate Cloud Flow
 participant Gateway as On-Premises Gateway
 participant DesktopFlow as Power Automate Desktop Flow
 participant LegacyCRM as Legacy Desktop CRM

 Note over User,LegacyCRM: RPA Integration Pattern: Bridging Tallyfy with Legacy Systems
 
 User->>Tallyfy: Complete "Client Info Update" task
 Tallyfy-->>CloudFlow: Trigger: Task completed event
 
 CloudFlow->>Tallyfy: Get task details (form field data)
 Tallyfy-->>CloudFlow: Return client data (phone, ID, etc.)
 
 CloudFlow->>Gateway: Run desktop flow with input variables
 Gateway->>DesktopFlow: Start RPA bot (attended/unattended)
 
 Note over DesktopFlow,LegacyCRM: UI Automation Sequence
 DesktopFlow->>LegacyCRM: Launch application
 DesktopFlow->>LegacyCRM: Click "Search Client"
 DesktopFlow->>LegacyCRM: Enter client ID
 DesktopFlow->>LegacyCRM: Click "Search"
 LegacyCRM-->>DesktopFlow: Display client record
 DesktopFlow->>LegacyCRM: Update phone field
 DesktopFlow->>LegacyCRM: Click "Save Changes"
 DesktopFlow->>LegacyCRM: Close application
 
 DesktopFlow-->>Gateway: Execution complete
 Gateway-->>CloudFlow: Success/failure status
 
 opt Update Tallyfy
 CloudFlow->>Tallyfy: Update process status (optional)
 end
```

**What to notice:**
- **Cloud-desktop separation** - Steps 1-4 happen in the cloud, while steps 7-15 execute on your local machine or server
...
- **UI automation sequence** - Steps 8-15 show the RPA bot mimicking human interactions with the legacy CRM interface
