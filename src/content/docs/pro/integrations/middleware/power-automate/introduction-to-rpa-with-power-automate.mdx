...
sequenceDiagram
    ...
    participant User
    participant Tallyfy
    participant CloudFlow as Power Automate<br/>Cloud Flow
    participant Gateway as On-Premises<br/>Gateway
    participant DesktopFlow as Power Automate<br/>Desktop Flow
    participant LegacyCRM as Legacy Desktop<br/>CRM

    Note over User,LegacyCRM: RPA Integration Pattern:<br/>Bridging Tallyfy with Legacy Systems
    
    User->>Tallyfy: Complete "Client Info Update" task
    Tallyfy-->>CloudFlow: Trigger: Task completed event
    
    CloudFlow->>Tallyfy: Get task details<br/>(form field data)
    Tallyfy-->>CloudFlow: Return client data<br/>(phone, ID, etc.)
    
    CloudFlow->>Gateway: Run desktop flow<br/>with input variables
    Gateway->>DesktopFlow: Start RPA bot<br/>(attended/unattended)
    ...
    opt Update Tallyfy
        CloudFlow->>Tallyfy: Update process status<br/>(optional)
    end
```
