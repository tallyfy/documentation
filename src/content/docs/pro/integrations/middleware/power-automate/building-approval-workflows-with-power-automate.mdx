...
```mermaid
flowchart TD
    Start([Tallyfy Task Completed:<br/>Submit Budget Proposal]) --> GetDetails[Get Task Details<br/>including Form Fields]
    GetDetails --> CheckAmount{Proposed Amount<br/>> $5,000?}
    
    CheckAmount -->|No| End1([Process Continues<br/>No Approval Needed])
    CheckAmount -->|Yes| GetManager[Get Manager<br/>from Office 365]
    
    GetManager --> SendApproval[Send Approval Request<br/>to Manager]
    SendApproval --> WaitResponse[Wait for<br/>Manager Response]
    
    WaitResponse --> CheckOutcome{Approval<br/>Outcome?}
    
    CheckOutcome -->|Approved| UpdateApproved[Update Tallyfy Task:<br/>'Budget approved by manager']
    CheckOutcome -->|Rejected| UpdateRejected[Update Tallyfy Task:<br/>'Budget rejected']
    
    UpdateRejected --> EmailSubmitter[Optional: Email<br/>Original Submitter]
    
    UpdateApproved --> End2([Process Continues])
    EmailSubmitter --> End3([Process Halted])

    style Start fill:#E8F4FF,stroke:#0066CC
    style End1 fill:#D4EDDA,stroke:#00AA55
    style End2 fill:#D4EDDA,stroke:#00AA55
    style End3 fill:#F8D7DA,stroke:#DC3545
