...
```mermaid
flowchart TD
    Start([Tallyfy Task Completed:<br/>Submit Budget Proposal]) --> GetDetails["`**Get Task Details**
including Form Fields`"]
    GetDetails --> CheckAmount{Proposed Amount<br/>> $5,000?}
    
    CheckAmount -->|"`No`"| End1([Process Continues<br/>No Approval Needed])
    CheckAmount -->|"`Yes`"| GetManager["`**Get Manager**
from Office 365`"]
    
    GetManager --> SendApproval["`**Send Approval Request**
to Manager`"]
    SendApproval --> WaitResponse["`**Wait for**
Manager Response`"]
    
    WaitResponse --> CheckOutcome{Approval<br/>Outcome?}
    
    CheckOutcome -->|"`Approved`"| UpdateApproved["`**Update Tallyfy Task:**
'Budget approved by manager'`"]
    CheckOutcome -->|"`Rejected`"| UpdateRejected["`**Update Tallyfy Task:**
'Budget rejected'`"]
    
    UpdateRejected --> EmailSubmitter["`**Optional: Email**
Original Submitter`"]
