---
description: Power Automate's conditional logic enables flows to make intelligent
  decisions based on Tallyfy data by using if-then-else structures that evaluate form
  fields task statuses and process information to automatically route different actions
  and create adaptive workflows that respond to varying business scenarios.
...
Trigger`"] --> GetDetails["`**Get Task**
Details`"]
    GetDetails --> Check{Approval<br/>= Approved?}
    
    Check -->|"`YES`"| Stakeholder["`**Email**
Stakeholder`"]
    Check -->|"`NO`"| Assignee["`**Email**
Assignee`"]
