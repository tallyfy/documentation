...
```mermaid
flowchart TD
    Start([Task Completed in Tallyfy]) --> Trigger[Power Automate Triggered<br/>Blueprint ID configured]
    Trigger --> GetDetails[Get Task Details<br/>Retrieve form fields]
    GetDetails --> CheckApproval{Approval Outcome<br/>equals 'Approved'?}
    
    CheckApproval -->|YES| ApprovedEmail[Send Email to Stakeholder<br/>Subject: Task Approved<br/>Include task details & link]
    CheckApproval -->|NO| ActionEmail[Send Email to Assignee<br/>Subject: Action Required<br/>Include outcome & task name]
    
    ApprovedEmail --> End([Flow Complete])
    ActionEmail --> End
    
    style Start fill:#E8F4FF,stroke:#0066CC
    style CheckApproval fill:#FFF3CD,stroke:#FFC107
    style ApprovedEmail fill:#D4EDDA,stroke:#00AA55
    style ActionEmail fill:#F8D7DA,stroke:#DC3545
    style End fill:#E8F4FF,stroke:#0066CC
    
    click GetDetails "/products/pro/tracking-and-tasks/tasks/" "Learn about Tallyfy tasks"
    click CheckApproval "/products/pro/tracking-and-tasks/tasks/what-are-form-fields-in-tallyfy/" "Understanding form fields"
```
