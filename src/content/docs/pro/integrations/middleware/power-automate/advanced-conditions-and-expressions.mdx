---
description: Power Automate's advanced conditional logic capabilities including expressions
  grouped conditions and Switch controls enable sophisticated decision-making for
  complex Tallyfy workflow integrations that handle multiple conditions calculations
  and edge cases beyond basic if-then scenarios.
...
```mermaid
flowchart TD
    Start([Tallyfy Task with<br/>Request Type Field]) --> Evaluate{Evaluate<br/>Request Type}
    
    Evaluate -->|"`Information`"| Info["`**Send email with**
KB article links`"]
    Evaluate -->|"`Access`"| Access["`**Create approval request**
in Teams`"]
    Evaluate -->|"`Hardware`"| Hardware["`**Launch Hardware**
Procurement template`"]
    Evaluate -->|"`Other/Unknown`"| Default["`**Notify IT support**
about unclassified request`"]
    
    Info --> End([Continue Flow])
    Access --> End
    Hardware --> End
    Default --> End
    
    
    click Hardware "/products/pro/documenting/templates/index/" "Learn about procedure templates"
    click Access "/products/pro/integrations/middleware/power-automate/integrating-power-automate-approvals-with-microsoft-teams/" "Teams approval integration"
```
