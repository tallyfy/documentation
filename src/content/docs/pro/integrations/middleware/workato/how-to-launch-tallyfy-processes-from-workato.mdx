...
sidebar:
  order: 1
snippet: Step-by-step guide to automatically launch Tallyfy processes from Workato recipes using the HTTP connector, including authentication setup, data mapping, and error handling for seamless workflow automation.
...
If your Tallyfy template has a [kick-off form](/products/pro/launching/triggers/via-kick-off-form/), you need to populate those fields in the `prerun` object:
...
<LinkTitleCard header="<b>Launching > Launch via middleware</b>" href="/products/pro/launching/triggers/via-middleware/" > Middleware platforms enable automated process launches in Tallyfy by connecting external software events to trigger specific workflows and transfer data between systems without coding requirements. </LinkTitleCard>
<LinkTitleCard header="<b>Templates > Variables</b>" href="/products/pro/documenting/templates/variables/" > Variables in Tallyfy templates enable dynamic content generation and data passing between process steps using system-defined and user-defined values that automatically populate during process execution. </LinkTitleCard>
<LinkTitleCard header="<b>Triggers > Launch via a kick-off form</b>" href="/products/pro/launching/triggers/via-kick-off-form/" > Kick-off forms collect essential information before launching a process enabling dynamic data capture customized workflows and streamlined process initiation through web forms or shareable links. </LinkTitleCard>
</CardGrid> 