...
sequenceDiagram
    autonumber
    participant ES as External System<br/>(Salesforce/Zendesk)
    ...
    ES->>W: Trigger Event<br/>(New opportunity, ticket, etc.)
    W->>W: Extract trigger data
    W->>HTTP: Configure request
    
    Note over HTTP: Authentication:<br/>Bearer token +<br/>X-Tallyfy-Client header
    
    HTTP->>T: POST /api/runs<br/>with JSON body
    Note right of T: Body includes:<br/>- blueprint_id<br/>- owner_id<br/>- prerun data
    
    alt Success
        T-->>HTTP: 200 OK<br/>Process details
