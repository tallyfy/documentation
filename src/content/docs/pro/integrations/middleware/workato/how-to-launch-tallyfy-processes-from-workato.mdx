---
description: Workato can automatically trigger Tallyfy workflow processes when external
  events occur in systems like Salesforce or Zendesk by using HTTP connectors that
  send properly formatted API requests with authentication headers and process data
  to launch templates with pre-populated kick-off forms.
