...
description: Tallyfy Analytics connects to Tableau Desktop through Amazon Athena in
  about 15 minutes. Install a JDBC driver and use Access Key credentials to query
  workflow data like process durations, task completion rates, and form field answers.
...
<Steps>
1. Download and install **64-bit Java** from [Java's official site](https://java.com/en/download/manual.jsp). The JDBC connection won't work without it.
...
:::note[Driver updates]
AWS updates their JDBC drivers periodically. Check their documentation for the latest version so you don't hit compatibility issues.
:::
...
3. Enter the **Server** as `athena.us-west-2.amazonaws.com`. All Tallyfy Analytics data lives in the us-west-2 (Oregon) region, so no other endpoint will work.
4. Enter the **Port** (`443`).
5. Enter the **S3 Staging Directory** exactly as provided by Tallyfy. It takes the form `s3://aws-athena-query-results-570841547480-us-west-2/<your-orgID>`, and it is where Athena stores temporary query results.
...
The **Region** has to match the one in your server endpoint exactly. A mismatch here is the most common reason Sign In fails.
...
If the Database dropdown is empty, your Athena user doesn't have permission on that catalog yet. Contact Tallyfy Support rather than picking a different catalog.
