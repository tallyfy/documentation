---
about_tallyfy: true
description: Sigma Computing cannot connect directly to Tallyfy Analytics due to a
  fundamental architecture mismatch between Sigma's need for persistent data warehouse
  connections and AWS Athena's serverless query model but you can bridge the gap using
  Snowflake or Redshift Spectrum to read Tallyfy's S3 data without duplication or
  switch to natively compatible BI tools like Power BI or Tableau or AWS QuickSight
  that support Athena directly.
