...
sidebar:
  order: 3
snippet: The CSV export feature provides a comprehensive 45-column data structure
  that captures process details tasks form fields and comments while enabling users
  to export process information through the settings panel for analysis and reporting
  purposes.
...
Tallyfy's CSV export provides a flat-file representation of process data. Each CSV file contains 45 columns that capture the complete state of a process, its tasks, form fields, and comments.
...
The column names in the CSV export use API terminology. While the Tallyfy user interface refers to standardized workflow designs as "Templates", the API and data exports use the term "Blueprint" for the same objects.
:::

| Column | Description | Data Type |
|--------|-------------|-----------|
| **blueprint_id** | Unique identifier for the template (uses API terminology) | String |
| **blueprint_name** | Name of the template used (uses API terminology) | String |
| **blueprint_version** | Version number of the template (uses API terminology) | Integer (as String) |
