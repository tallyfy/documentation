---
about_tallyfy: true
description: Connect Power BI to your Tallyfy workflow data through Amazon Athena
  using an ODBC driver in about 15 minutes. Build dashboards from per-process Parquet
  files using your Access Key and Secret Key in us-west-2. Set the driver's Authentication
  Option to IAM Credentials, which is the option that accepts a typed key.
...
:::caution[The dropdown named "IAM Credentials" is the right one]
This is the one genuinely confusing part of the setup. The Athena ODBC driver has an Authentication Option called **IAM Credentials**, and that is the option you want, because it is the one that accepts a typed Access Key and Secret Key. It has nothing to do with the username and password you use to sign in to the AWS console, which the driver cannot use at all.

Do not pick **IAM Profile**. That option reads credentials from an instance profile or a local AWS config file, so there is nowhere to type your key and the connection will fail.
:::

:::note[If you see "The security token included in the request is invalid"]
Check your **Workgroup** setting first. Left at the default `primary`, the driver produces exactly this message even though your key is correct. Set it to your organization ID and try again. If the message persists, confirm you pasted the Access Key and Secret Key rather than your console username and password.
:::
...
<Steps>
1. Download the Amazon Athena ODBC driver **version 2** from the [AWS ODBC 2.x driver page](https://docs.aws.amazon.com/athena/latest/ug/odbc-v2-driver.html). Version 1 is the older Simba-branded driver; version 2 is the current one
...
:::caution[Use a System DSN, not a User DSN]
On the gateway machine the DSN must be created on the **System DSN** tab. A User DSN produces the error "Data source name not found and no default driver specified" when the gateway tries to connect.

The reason is that User DSNs live in the `HKEY_CURRENT_USER` registry hive, and the gateway runs as a Windows service under an account that never loads that hive. This is a Windows limitation rather than anything specific to Athena.
:::
...
<LinkTitleCard header="<b>Analytics > How Tallyfy Analytics works</b>" href="/products/pro/integrations/analytics/how-does-tallyfy-analytics-process-my-data/" > Tallyfy Analytics processes your workflow data through a five-step pipeline, converting events... </LinkTitleCard>
</CardGrid>