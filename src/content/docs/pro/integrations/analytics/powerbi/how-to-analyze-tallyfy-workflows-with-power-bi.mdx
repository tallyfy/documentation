...
flowchart TD
    ...
    CheckAuth -->|Recommended| AccessKey[Access Key +<br/>Secret Access Key]
    CheckAuth -->|Not Recommended| IAM[IAM Username/<br/>Password]
    
    AccessKey --> ODBC[Configure ODBC<br/>Data Source]
    IAM --> Error[❌ Security Token<br/>Invalid Error]
    
    ODBC --> Settings[Settings:<br/>• Region: us-west-2<br/>• Workgroup: Org ID<br/>• Catalog: AwsDataCatalog]
    
    Settings --> Athena[Amazon Athena<br/>Connection]
    
    Athena --> Data[(Tallyfy Analytics Data)]
    
    Data --> Tables[Available Tables:<br/>• processes<br/>• steps<br/>• form_fields<br/>• users]
    
    Tables --> Dashboard[Create Power BI<br/>Dashboards]
    
    style Start fill:#E8F4FF,stroke:#0066CC
    style Dashboard fill:#D4EDDA,stroke:#00AA55
    style Error fill:#F8D7DA,stroke:#DC3545
    style AccessKey fill:#D4EDDA,stroke:#00AA55
