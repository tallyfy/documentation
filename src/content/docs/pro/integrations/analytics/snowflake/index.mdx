---
about_tallyfy: true
description: Snowflake can query Tallyfy Analytics data stored as Parquet files in
  Amazon S3. Set up an external stage with your AWS credentials and run SQL queries
  against process and member data with no ETL pipeline needed.
id: b939b770b3791368c812d77d653d7afd
...
:::note[Prerequisites]
Tallyfy Analytics must be enabled for your organization. If you don't have analytics credentials yet, contact your Tallyfy account manager to get set up.
:::
...
If your Snowflake account doesn't use IAM role-based access, you can skip the storage integration and provide your AWS credentials directly in the stage definition (shown in Step 2 below). Key-based authentication is simpler and works well for most Tallyfy Analytics setups.
:::
...
### What member data is available

Member records are not part of Tallyfy Analytics. The credentials issued with your
Analytics account can read your process and task data only, so a query against a
members folder returns an access error rather than rows.

Where you need who did what, the run analytics data already carries it. Every task
row names the person it was assigned to and the person who completed it, which
covers most reporting on people without needing a separate member export.
...
**Data format** - All fields in the Parquet files are cast to STRING. You'll need to cast dates and numbers in your Snowflake queries as needed.
...
**Data freshness** - Analytics data is updated when process events occur (task completions, status changes, form submissions). There's no fixed refresh schedule - data appears as events happen.

**Existing Athena access** - Snowflake and Athena can both read the same S3 data simultaneously. Adding Snowflake doesn't affect your existing Athena, Power BI, Tableau, or Looker connections.
...
## Related articles
<CardGrid>
<LinkTitleCard header="<b>Analytics > Snowflake</b>" href="/products/pro/integrations/analytics/snowflake/" > Connect Snowflake to your Tallyfy Analytics data stored in Amazon S3 by creating a storage... </LinkTitleCard>
