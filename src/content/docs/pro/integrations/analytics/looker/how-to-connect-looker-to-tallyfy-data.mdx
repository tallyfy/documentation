...
description: Looker connects to Tallyfy's workflow analytics data through Amazon Athena
  in about 15-20 minutes by installing a JDBC driver (for self-hosted instances) and
  configuring an Athena connection with AWS Access Key credentials to query Parquet-formatted
  process and task data stored in S3 and then build LookML models and dashboards for
  tracking process durations and team performance and SLA compliance.
