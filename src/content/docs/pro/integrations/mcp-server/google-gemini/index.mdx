---
about_tallyfy: true
description: Google Gemini connects to Tallyfy's MCP server through OAuth 2.1 and
  streamable HTTP transport. Use the Gemini CLI for local connections or Google Cloud's
  managed MCP infrastructure for enterprise deployments that combine Tallyfy data
  with BigQuery and other Google services.
...
The MCP server is hosted at `https://mcp.tallyfy.com/` and listed on the [Official MCP Registry](https://registry.modelcontextprotocol.io/v0/servers?search=tallyfy) as `com.tallyfy/mcp-server`. Gemini connects via the standard streamable-http transport with OAuth 2.1.
:::

Google Gemini connects to Tallyfy's MCP server through OAuth 2.1, letting you manage workflows with natural language. You can use the Gemini CLI for local connections or Gemini Enterprise's custom MCP data store for enterprise deployments.

## Gemini MCP support status

- **Official support** - MCP support announced December 2025 for Gemini models
- **Gemini CLI** - FastMCP-style MCP server connections
- **Gemini Enterprise** - Custom MCP server data store, configured per Gemini Enterprise deployment
