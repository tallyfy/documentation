---
about_tallyfy: true
description: Connect Tallyfy's MCP server to Gemini CLI, Gemini Code Assist, and Gemini Enterprise. Run OAuth 2.1 with Dynamic Client Registration for the CLI surfaces, and add Tallyfy as a Custom MCP Server data store for enterprise teams.
...
4. **Verify the connection**
   ...
   You'll see `tallyfy` listed with 113 tools.
...
3. **Register an OAuth client for Tallyfy**
   ...
   Tallyfy supports Dynamic Client Registration. Generate a client_id for Gemini Enterprise with one curl call:
   ...
   The response contains a `client_id`. Save it.

   <Aside type="caution" title="Client secret workaround">
   Tallyfy's OAuth server currently uses PKCE-only public clients, so the DCR response doesn't include a `client_secret`. The Gemini Enterprise console marks **Client Secret** as a required field. As a workaround, enter a non-empty placeholder string like `pkce-public-client-no-secret` - Tallyfy's token endpoint ignores the secret because PKCE replaces it. We're tracking confidential-client support; if the console rejects the placeholder, [file an issue](https://github.com/tallyfy/mcp/issues) with the exact error message.
   </Aside>
...
4. **Add the data store in Cloud Console**
   ...
   | **Client Secret** | `pkce-public-client-no-secret` (see workaround above) |
...
- **PKCE-only OAuth today** - Tallyfy's OAuth server is currently public-client only. See the client secret workaround in the Enterprise setup steps above.
...
- **Authentication** - OAuth 2.1. Gemini CLI and Code Assist use PKCE with S256 code challenge through DCR. Gemini Enterprise also uses PKCE today (see the workaround note above for the client secret field). Refresh tokens rotate automatically.
