...
description: Google Gemini connects to Tallyfy's MCP server through OAuth 2.1 and
  streamable HTTP transport enabling users to manage workflows and tasks and templates
  with natural language via either the Gemini CLI for local connections or Google
  Cloud's managed MCP infrastructure for enterprise deployments that can also combine
  Tallyfy process data with BigQuery and other Google services.
