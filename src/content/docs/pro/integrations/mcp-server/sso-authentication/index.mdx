---
description: SSO integration with MCP servers creates unified authentication for enterprise
  AI workflows by letting users access multiple integrated tools through their corporate
  credentials instead of managing separate logins for each AI application.
...
AI: AI Agent
MCP: MCP Server
IdP: Identity Provider (Azure AD/Okta)
API: Target Service API

# SSO-MCP Authentication Flow

1: U -> AI: Request access to tool
2: AI -> MCP: Forward request
3: MCP -> U: Redirect to IdP login
4: U -> IdP: Corporate credentials + MFA
5: IdP -> IdP: Validate identity\nApply policies
6: IdP -> U: Authorization code
7: U -> MCP: Return with auth code
8: MCP -> IdP: Exchange code for tokens
9: IdP -> MCP: Access token + Refresh token
10: MCP -> API: API request with Bearer token
11: API -> MCP: Protected data
12: MCP -> AI: Process response
13: AI -> U: Display results

# Notes
SSO-MCP Authentication Flow: {near: top-center}
Token lifecycle managed by MCP server: {near: bottom-center}
```
