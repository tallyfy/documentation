...
- **Keep docs and templates in sync.** Paste an updated procedure and ask ChatGPT to update the matching template. It points out exactly what changed.
...
3. **Authenticate via OAuth 2.1**
   ...
   During authorization, you'll select which organization to connect and approve scopes like `mcp.tasks.read`, `mcp.processes.write`, etc. If your account isn't part of a Tallyfy workspace yet, you'll see a prompt to create one first, then you're brought back to finish connecting.
...
What happens: ChatGPT retrieves the template structure using `get_template`, lists all steps with descriptions, documents form fields for each step, and includes the automations.
