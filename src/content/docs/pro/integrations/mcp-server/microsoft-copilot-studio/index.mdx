...
description: Microsoft Copilot Studio provides enterprise-grade MCP support that connects
  Tallyfy's workflow management with intelligent agents through the Power Platform
  infrastructure enabling multi-agent orchestration automated compliance workflows
  and deep integration with Microsoft's ecosystem while requiring additional setup
  complexity and licensing considerations for organizations seeking AI-powered business
  process automation.
...
sequenceDiagram
    ...
    Note over Dev,T: Setup Flow
    
    Dev->>Azure: Deploy server
    Dev->>PA: Create connector
    PA->>PA: Configure auth
    Dev->>CS: Add connector
    CS->>PA: Authorize
    PA->>T: Test API
    T-->>PA: Verified
    CS->>CS: Configure DLP
...
%%{init: {'theme':'neutral'}}%%
flowchart TD
    Start([Onboard Request]) --> Primary[Primary Agent]
    
    Primary --> Analyze{Analyze}
    
    Analyze -->|HR| HR[HR Agent]
    Analyze -->|IT| IT[IT Agent]  
    Analyze -->|Training| Train[Training Agent]
    
    HR --> HR1[Create Tasks]
    IT --> IT1[Setup Access]
    Train --> T1[Schedule Training]
    
    HR1 --> Monitor[Monitor Progress]
    IT1 --> Monitor
    T1 --> Monitor
    
    Monitor --> Status[Status Report]
    Status --> End([Complete])
