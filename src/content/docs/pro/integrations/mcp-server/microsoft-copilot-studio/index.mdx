...
- A Tallyfy credential: either your Tallyfy API key from your organization settings, or a Tallyfy account to authorize through OAuth. The steps below use the API key, because it is the quicker of the two to wire up.
...
The MCP server is hosted at `https://mcp.tallyfy.com/` and exposes the full Tallyfy tool set. It's listed on the [Official MCP Registry](https://registry.modelcontextprotocol.io/?q=tallyfy) as `com.tallyfy/mcp-server`. Copilot Studio connects to it over streamable HTTP.
...
The server accepts **two** credentials, and both work today. Pick one and keep the whole connector consistent with it.

| | Tallyfy API key | OAuth 2.1 |
|---|---|---|
| How it travels | `Authorization: Bearer <your Tallyfy API key>` | `Authorization: Bearer <OAuth access token>` |
| Connector security scheme | `apiKey`, header `Authorization`, prefix `Bearer ` | `oauth2`, authorization code flow with PKCE |
| Setup effort | Low. Paste one value and test. | Higher. Register the connector against the authorization server first. |
| Per-user identity | No. Every agent call acts as whoever owns the key. | Yes. Each person authorizes with their own Tallyfy account. |
| Scoped permissions | No. The key carries the owner's full access. | Yes. Twelve scopes, in dot notation like `mcp.tasks.read` and `mcp.processes.write`. |
| Revocation | Rotate the key in organization settings. | Revoke that user's grant. |
| Best for | Getting a connector working, and single-owner automation. | Shared agents, and anywhere per-user audit trails or least privilege matter. |

The steps below build the API key version, because it is the shortest path to a working connector. If you need per-user identity or scoped permissions, use OAuth instead. Discovery starts at `/.well-known/oauth-authorization-server` (per RFC 8414), which advertises `https://mcp.tallyfy.com/mcp/oauth/authorize` and `https://mcp.tallyfy.com/mcp/oauth/token`, with PKCE `S256`.

Whichever you choose, an unauthenticated or invalid request is rejected with `401 invalid_token`. There is no anonymous access.

### Prerequisites

- Microsoft Copilot Studio license (trial or paid)
- A Tallyfy API key from your organization settings, if you are following the steps below. For the OAuth route, a Tallyfy account to authorize with instead.
...
1. **Connect to Tallyfy's MCP server**
   ...
   Tallyfy provides a hosted MCP server at `https://mcp.tallyfy.com` that exposes tools for managing tasks, processes, templates, users, form fields, kickoff fields, automation, and more. You don't need to build your own MCP server - instead, you'll create a connector that points to Tallyfy's existing server.

   These steps build the API key version of the connector. See [Which credential to use](#which-credential-to-use) if you need OAuth instead.
...
4. **Configure authentication**
   ...
   A successful test returns the server's tool list. A wrong or missing credential returns `401 invalid_token`, so a green test really does mean the credential was accepted.

   For OAuth instead, set the security scheme to `oauth2` with the authorization code flow, point it at the two endpoints listed under [Which credential to use](#which-credential-to-use), and request the scopes your agent needs. Tallyfy also publishes an authored Power Platform connector using OAuth, which is a useful reference if you go this route.
