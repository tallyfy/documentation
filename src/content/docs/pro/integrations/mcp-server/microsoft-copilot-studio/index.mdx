---
description: Microsoft Copilot Studio brings enterprise-grade MCP support to organizations
  connecting Tallyfy's workflow automation with intelligent agents that handle complex
  multi-agent orchestration compliance management and deep Power Platform integration
  while requiring additional licensing and setup complexity compared to simpler alternatives.
...
%%{init: {'theme':'neutral'}}%%
flowchart TD
    Start([User Request:<br/>Onboard Jane Smith]) --> Primary["`**Primary Agent**
Orchestrator`"]
...
Training Sessions`"]
    ...
    HR1 --> Monitor["`**Monitor Progress**
in Tallyfy`"]
    IT1 --> Monitor
    T1 --> Monitor
    
    Monitor --> Status["`**Consolidated**
Status Report`"]
    Status --> End([Onboarding<br/>Complete])
    
    click HR1 "/products/pro/tracking-and-tasks/tasks/index/" "Task management"
    click Monitor "/products/pro/tracking-and-tasks/tracker-view/index/" "Process tracking"
```
...
```mermaid
graph TB
    Copilot[Copilot Studio] --> Gateway["`**API Gateway**
+ WAF`"]
    ...
    MCP1 --> API[Tallyfy API]
    MCP2 --> API
    
    click API "/products/pro/integrations/open-api/index/" "API documentation"
    click Copilot "#prerequisites" "Setup prerequisites"
```
