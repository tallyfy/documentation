...
The MCP server is hosted at `https://mcp.tallyfy.com/` and exposes 108 tools. It's listed on the [Official MCP Registry](https://registry.modelcontextprotocol.io/?q=tallyfy) as `com.tallyfy/mcp-server`. Copilot Studio connects via streamable HTTP with OAuth 2.1.
:::
