...
sequenceDiagram
    ...
    Note over Dev,T: Authentication & Setup Flow
    
    Dev->>Azure: Deploy MCP server code
    Dev->>PA: Create custom connector<br/>with OpenAPI spec
    PA->>PA: Configure API Key auth
    Dev->>CS: Add MCP connector
    CS->>PA: Authorize connection
    PA->>T: Test with API key
    T-->>PA: Connection verified
    CS->>CS: Configure DLP policies
    CS->>T: Execute test queries
    T-->>CS: Results returned
    
    Note over CS,T: Ready for production use
```
...
%%{init: {'theme':'neutral'}}%%
flowchart TD
    Start([User Request:<br/>Onboard Jane Smith]) --> Primary[Primary Agent<br/>Orchestrator]
    
    Primary --> Analyze{Analyze<br/>Request}
    
    Analyze -->|HR Tasks| HR[HR Agent]
    Analyze -->|IT Tasks| IT[IT Agent]  
    Analyze -->|Training| Train[Training Agent]
    
    HR --> HR1[Create Tallyfy<br/>Documentation Tasks]
    IT --> IT1[Setup System<br/>Access via Tallyfy]
    Train --> T1[Schedule<br/>Training Sessions]
    
    HR1 --> Monitor[Monitor Progress<br/>in Tallyfy]
    IT1 --> Monitor
    T1 --> Monitor
    
    Monitor --> Status[Consolidated<br/>Status Report]
    Status --> End([Onboarding<br/>Complete])
    
    style Start fill:#E8F4FF,stroke:#0066CC
    style End fill:#D4EDDA,stroke:#00AA55
    style Primary fill:#FFF3CD,stroke:#FFC107
    
    click HR1 "/products/pro/tracking-and-tasks/tasks/" "Task management"
    click Monitor "/products/pro/tracking-and-tasks/tracker-view/" "Process tracking"
```
...
graph TB
    ...
    CS -->|VNet Integration| APIM[API Management<br/>Gateway]
    
    subgraph "Azure App Service Environment"
        APIM -->|Private Endpoint| LB[Load Balancer]
        LB --> MCP1[MCP Server 1<br/>Instance]
        LB --> MCP2[MCP Server 2<br/>Instance]
    end
    
    MCP1 -->|Managed Identity| API[Tallyfy API<br/>Endpoints]
    MCP2 -->|Managed Identity| API
    ...
    style CS fill:#E8F4FF,stroke:#0066CC
    style API fill:#D4EDDA,stroke:#00AA55
    style WAF fill:#F8D7DA,stroke:#DC3545
    
    click API "/products/pro/integrations/open-api/" "API documentation"
    click CS "#prerequisites" "Setup prerequisites"
```
