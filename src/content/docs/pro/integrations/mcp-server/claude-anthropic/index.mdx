---
description: Claude Desktop's MCP integration with Tallyfy transforms AI workflow
  management by connecting natural language queries to process automation through
  mature tooling that includes one-click Desktop Extensions automated credential management
  and the industry's best coding capabilities while the exploding ecosystem of 7,260+
  MCP servers and major platform adoption makes it a powerful complement to visual
  workflow interfaces.
...
1: User -> Claude: Launch Claude Desktop
2: Claude -> Config: Read claude_desktop_config.json
3: Config --> Claude: MCP server configuration {style.stroke-dash: 3}
4: Claude -> MCP: Start server via stdio
5: MCP -> MCP: Load TALLYFY_API_KEY from env
6: MCP --> Claude: Server ready {style.stroke-dash: 3}

7: Claude -> MCP: Request tools/list
8: MCP --> Claude: Available tools (search_tasks, etc.) {style.stroke-dash: 3}
9: Claude -> Claude: Display tools in UI (🔨)

# Usage Phase
Usage Phase: {near: middle-center}

10: User -> Claude: "Search for open tasks"
11: Claude -> MCP: tools/call: search_tasks {query: "", status: "open"}
12: MCP -> API: GET /tasks Bearer: [API_KEY]
13: API --> MCP: Task data (JSON) {style.stroke-dash: 3}
14: MCP --> Claude: Formatted results {style.stroke-dash: 3}
15: Claude --> User: Display task summary {style.stroke-dash: 3}
```
