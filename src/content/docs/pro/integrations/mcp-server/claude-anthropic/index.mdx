---
description: Claude Desktop has evolved into the most mature MCP platform with Desktop
  Extensions for one-click installation Remote MCP servers with OAuth and Claude 4
  models achieving 72.5% SWE-bench accuracy enabling powerful natural language workflow
  automation with Tallyfy while the entire AI ecosystem including OpenAI Microsoft
  and Google has standardized on MCP creating an explosive growth from 1000 to 7260+
  available servers by August 2025.
...
Config: Config File
MCP: MCP Server (tallyfy-server.js)
API: Tallyfy API

# Initialization Phase

User -> Claude: "1. Launch Claude Desktop"
Claude -> Config: "2. Read claude_desktop_config.json"
Config --> Claude: "3. MCP server configuration" {style.stroke-dash: 3}
Claude -> MCP: "4. Start server via stdio"
MCP -> MCP: "5. Load TALLYFY_API_KEY from env"
MCP --> Claude: "6. Server ready" {style.stroke-dash: 3}

Claude -> MCP: "7. Request tools/list"
MCP --> Claude: "8. Available tools (search_tasks, etc.)" {style.stroke-dash: 3}
Claude -> Claude: "9. Display tools in UI (🔨)"

# Usage Phase

User -> Claude: "10. Search for open tasks"
Claude -> MCP: "11. tools/call: search_tasks {query: '', status: 'open'}"
MCP -> API: "12. GET /tasks Bearer: [API_KEY]"
API --> MCP: "13. Task data (JSON)" {style.stroke-dash: 3}
MCP --> Claude: "14. Formatted results" {style.stroke-dash: 3}
Claude --> User: "15. Display task summary" {style.stroke-dash: 3}
```
