...
sequenceDiagram
 ...
 participant MCP as MCP Server (tallyfy-server.js)
 ...
 MCP-->>Claude: Available tools (search_tasks, etc.)
 Claude->>Claude: Display tools in UI (🔨)
 
 Note over User,API: Usage Phase
 
 User->>Claude: "Search for open tasks"
 Claude->>MCP: tools/call: search_tasks {query: "", status: "open"}
 MCP->>API: GET /tasks Bearer: [API_KEY]
...
<LinkTitleCard header="<b>Vendors > Claude computer use</b>" href="/products/pro/integrations/computer-ai-agents/vendors/claude-computer-use/" > Claude can now directly control computers by clicking buttons typing text and navigating applications through its Computer Use capability which works with Claude 4 models Claude 3.7 Sonnet and Claude 3.5 Sonnet v2 to automate repetitive UI tasks that integrate with Tallyfy workflows through an intermediary application that connects webhooks to the Anthropic API for secure sandboxed execution of desktop and web application automation. </LinkTitleCard>
