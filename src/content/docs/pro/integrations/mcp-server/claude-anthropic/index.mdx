...
Claude Desktop was the first AI assistant to offer native MCP integration. The Anthropic family that supports remote MCP today includes Claude Opus, Sonnet, and Haiku models on Pro, Max, Team, and Enterprise plans, plus Claude Code in VS Code, JetBrains, and GitHub Actions.

This guide covers setting up Tallyfy's MCP server with Claude Desktop, claude.ai connectors, and Claude Code. The setup uses remote OAuth, no local installation required.
...
## Live examples in Claude

Real screenshots from claude.ai with the Tallyfy connector active. Each one shows a different MCP capability in action.

### Template creation from a BPMN diagram

A user dropped a BPMN diagram into Claude and asked for a matching template. Claude used `create_template`, `add_step_to_template`, and `create_automation_rule` to build a 7-step Business Trip Request with 5 if-then rules - approval gates, parallel notification, conditional show/hide on rejection.
...
### Clarifying questions before pulling data

Claude doesn't guess. When the question is ambiguous, the `ask_user_question` MCP tool surfaces a structured picker so you confirm scope before any tools fire. Here Claude needs to know whose tasks, what counts as pending, and how to define "next month" before it'll touch your account.

![Claude.ai Cowork sidebar showing the Tallyfy connector connected, with Claude asking four clarifying questions via the ask_user_question MCP tool before querying pending tasks](https://screenshots.tallyfy.com/tallyfy/pro/mcp-claude-clarifying-questions.png)

### Reviewing an existing process

Asked to review a Client Onboarding process and suggest improvements, Claude first confirms what you actually want - suggest only, suggest then edit on approval, or edit directly. The 4-option picker comes from `ask_user_to_rank` and the response routes the rest of the run.

![Claude.ai Cowork sidebar reviewing a Client Onboarding process and using ask_user_question to confirm scope - four options including suggest only, edit on approval, edit directly, or something else](https://screenshots.tallyfy.com/tallyfy/pro/mcp-claude-process-review.png)

### Multi-tool analysis with progress tracking

The right-side progress sidebar shows Claude chaining `get_me`, `get_my_tasks`, then counting + verifying. End result: 822 open tasks, 805 explicitly flagged overdue by Tallyfy, 0 due in the next 30 days because everything is already past its date. Claude flagged the surprise and offered three follow-up paths instead of just answering the literal question.
