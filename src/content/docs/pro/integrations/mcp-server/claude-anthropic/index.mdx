---
about_tallyfy: true
description: Connect Claude to Tallyfy's hosted MCP server to manage workflows in
  plain language. Add it as a custom connector in Claude.ai or Claude Desktop, sign
  in with OAuth, then query tasks, analyze processes, and combine with Computer Use
...
A user dropped a flowchart into Claude and asked for a matching template. Claude built a 7-step Business Trip Request with 5 automations - approval gates, parallel notifications, and conditional show/hide on rejection.
...
- A Tallyfy account that belongs to a workspace (organization) - you authorize through OAuth, so there's no API key to paste. If you sign in but don't have a workspace yet, Tallyfy walks you through creating one before finishing the connection.
...
### Connect the Tallyfy MCP server
...
1. **Open settings**
   ...
   Go to [claude.ai](https://claude.ai), click your profile icon, then **Settings**.

2. **Add a custom connector**

   Open **Connectors**, click **Add custom connector**, and enter the URL `https://mcp.tallyfy.com`.

3. **Authorize with Tallyfy**
...
1. **Update Claude Desktop**
   ...
   Download the latest version from [claude.ai/download](https://claude.ai/download). Remote connectors with OAuth need a recent build.

2. **Add the server**

   Open **Settings**, go to **Connectors** (labelled **Integrations** in some versions), and add the URL `https://mcp.tallyfy.com`.

3. **Authorize and restart**
...
For the full step-by-step with screenshots, see [Claude integration](/products/pro/integrations/byo-ai/claude-integration/).
...
2. **MCP server isolation**
   ...
   - Remote MCP uses OAuth 2.1 with PKCE. Tallyfy's endpoints:
     - Authorization: `https://mcp.tallyfy.com/mcp/oauth/authorize`
     - Token: `https://mcp.tallyfy.com/mcp/oauth/token`
     - Dynamic Client Registration: `https://mcp.tallyfy.com/mcp/oauth/register`
   - OAuth discovery via `https://mcp.tallyfy.com/.well-known/oauth-authorization-server` (RFC 8414), so Claude finds these automatically and you rarely need them by hand
...
#### Other MCP servers alongside Tallyfy
