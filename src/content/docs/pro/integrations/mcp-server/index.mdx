...
Both endpoints share the same OAuth backend, the same 109 tools, and the same Tallyfy organization data. Pick `mcp.tallyfy.com` by default; use `mcp-gcp.tallyfy.com` when your AI platform (Gemini Enterprise in particular) prefers a Google Cloud-hosted origin.
