...
- `assess_template_health` - Retrieve complete template data for a comprehensive health assessment by the AI
...
- `analyze_template_automations` - Review all automation rules with redundancy detection (exact duplicates, same-trigger groups)
- `suggest_automation_consolidation` - Get prioritized recommendations for consolidation — detects orphaned rules, duplicates, conflicts, and merge candidates
- `get_step_visibility_conditions` - Check when steps appear or hide based on automation rules
...
- **Tool system** - Structured tool definitions that tell AI clients what actions are available
