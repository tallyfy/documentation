...
From that point, the process runs like any other - Tallyfy assigns tasks to the right people the way the template says to, calculates deadlines, and sends notifications. The only difference from launching manually is that you never touched the Tallyfy interface.
