...
Underneath, `launch_process` posts to `POST /organizations/{org_id}/runs`. The kick-off values travel in a `prerun` object keyed by each field's timeline ID, a 32-character hex string:
...
```json
{
  ...
  "prerun": {
    "a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6": "Acme Corp",
    "3c9d1e7fa4b820516d8e2f7a9c0b4d15": { "id": 2, "text": "Enterprise" },
    "9f2b7c1e4a6d8035b1c7e9d2f4a6b801": "jane@acme.com"
  }
}
```

That's why the `get_kickoff_fields` call in step 2 matters so much: it's where your AI picks up those IDs. A key that isn't a timeline ID matches no field, and Tallyfy drops it without raising an error. The process would launch looking perfectly healthy with the value quietly missing.

The value format depends on the field type:

| Field type | Value to send |
|------------|---------------|
| Short text, long text, email | A plain string |
...
Watch the multi-select shape in particular. Drop the `"selected": true` flag and the value still saves without an error, but it renders as empty text everywhere the field is used as a `{{variable}}`. Tell your AI to include the flag on every option it picks.

Each `prerun` value's shape depends on the field type. The per-type list lives on [Launch process](/products/pro/integrations/open-api/code-samples/processes/launch-process/).
