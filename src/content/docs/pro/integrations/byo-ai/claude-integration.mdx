---
about_tallyfy: true
description: Claude connects to Tallyfy's MCP server at mcp.tallyfy.com using OAuth
  2.1 with PKCE. Manage tasks, processes, templates, and automations through natural
  language in Claude.ai or Claude Desktop with over 100 tools.
...
You connect once by adding Tallyfy as a connector in Claude's settings, then signing in with your Tallyfy account. Claude takes it from there. The exact steps for the web app and the desktop app are under [For developers](#for-developers).
...
- **Manage form fields and automations** - add or change fields, and create or clean up the automations that drive your workflows.
- **Work with comments and people** - read and add task comments, and look up or invite team members and guests.

### How to connect

The setup is quick. In Claude's settings, open **Connectors** (or **Integrations** in the desktop app), add a custom connector named Tallyfy pointing at the Tallyfy MCP server[^1], and sign in with your Tallyfy account to authorize it. Add it once and it syncs across Claude on the web, desktop, and mobile. Step-by-step clicks for both apps are under [For developers](#for-developers).
...
Yes. Anthropic doesn't train on your data. The Claude Desktop app stores your sign-in token in your computer's secure keychain (macOS Keychain or Windows Credential Manager), never in plain text. Each organization's data stays separate, every interaction is logged for audit, and Anthropic maintains SOC 2 Type II compliance. The sign-in detail is under [For developers](#for-developers).

Here's the trust boundary in one picture: what crosses from you into Tallyfy, and what stays on Tallyfy's side.

```d2
# How Claude reaches your Tallyfy data
direction: down
...
Tallyfy: Tallyfy {
  ...
  Data: Your Tallyfy Data {
    style.fill: "#f2faf4"
    style.stroke: "#225930"
    style.stroke-width: 2
  }

  Gate -> Data: Reads and writes {
    style.stroke: "#225930"
  }
}

You -> Claude: Chats in plain English {
  style.stroke: "#225930"
}
Claude -> Tallyfy: Signs in once {
  style.stroke: "#225930"
  style.stroke-width: 2
}
```
...
**What to notice:**
- You sign in once. Your sign-in token is stored securely, never as plain text.
- The MCP server sits inside Tallyfy's boundary, and Anthropic doesn't train on what it sees.
- Each organization's Tallyfy data stays separate from every other's.
...
| `mcp.automation.read` | Read automations |
| `mcp.automation.write` | Create and modify automations |

### MCP server tools

These are the tools Claude reaches for most often. For every tool on the server, see [every tool the MCP server gives your AI](/products/pro/integrations/mcp-server/tool-list/). That page is generated from the running server, so it is always the current list.
...
- `create_standalone_task` - Create a one-off task from a title, a deadline, and at least one assignee. The deadline can be everyday wording like "Friday at 5pm", and Tallyfy turns it into a real date and time in your organization's timezone
...
- `launch_process` - Launch a new process from a template with a name, optional tags/folders, and optional `prerun` values to pre-fill the kick-off form. The `prerun` object is keyed by each kick-off field's timeline ID (a 32-character hex string), not by the field's label or alias. Claude gets those IDs from `get_kickoff_fields`, so let it make that call rather than supplying field names yourself
...
**Automation tools**
- `create_automation_rule` - Create an automation on a template
- `update_automation_rule` - Modify an existing automation's conditions or actions
- `delete_automation_rule` - Remove an automation
...
- `suggest_automation_consolidation` - Get prioritized recommendations for consolidation (orphaned automations, duplicates, conflicts, merge candidates)
...
- `get_template_folders` - Get the folders that organize templates
- `get_process_folders` - Get the folders that organize processes
...
suggest improvements to the automations."
...
3. **Add human checkpoints** - For write operations like creating automations, review Claude's plan before confirming
