...
This integration is live and operational. OAuth 2.1 endpoints are active, and ChatGPT's Apps platform (formerly called "connectors" - renamed December 2025) fully supports MCP servers.
:::

## Connecting ChatGPT to automate Tallyfy tasks

Your ChatGPT Plus or Team subscription can become a workflow automation engine inside Tallyfy. Through Apps and OAuth2 authentication, ChatGPT reads your task data, processes it according to your instructions, and completes work automatically - all using the subscription you already pay for.

### How ChatGPT integration works

The integration uses OpenAI's Apps platform (formerly GPT Actions/connectors) to create a secure bridge between your ChatGPT account and Tallyfy workflows:
...
ChatGPT uses OAuth 2.1 with PKCE for secure authentication. All endpoints are live and operational:
...
1. **Verify subscription**: Ensure you have ChatGPT Plus, Team, Enterprise, or Education
2. **Enable Developer Mode**: In ChatGPT Settings → Beta Features → Developer Mode (required for write operations)
3. **Add Tallyfy as an App**: Navigate to Settings → Apps and add the Tallyfy MCP server
4. **Configure OAuth**: Authorize connection to your Tallyfy workspace using the OAuth flow
5. **Test with simple task**: Create a basic automation to verify setup
6. **Add to templates**: Embed AI instructions in your workflow templates
7. **Monitor and refine**: Review AI performance and adjust instructions

**For developers**: OpenAI's Apps SDK (developers.openai.com/apps-sdk) allows building custom UIs alongside MCP servers for enhanced user experiences.
