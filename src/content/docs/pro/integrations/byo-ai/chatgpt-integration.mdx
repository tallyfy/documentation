...
description: ChatGPT users on Plus or higher plans can connect to Tallyfy's MCP server
  ...
  through plain language with over 100 available tools.
...
- **Set up automations** - create, change, delete, and review the automations on a template, with suggestions for cleaning up duplicates and conflicts.
...
The full list of actions, with exact names, is on [every tool the MCP server gives your AI](/products/pro/integrations/mcp-server/tool-list/). That page is generated from the running server, so it is always current. The most-used ones are also in the [For developers](#for-developers) section below.
...
- **"Add an automation to the Sales template that assigns the contract review step to Sarah when a deal is over $50,000."** ChatGPT finds the step, sets up the condition and the action, and creates it after you approve.
...
| `mcp.automation.read` | View automations |
| `mcp.automation.write` | Create and modify automations |

### The most-used MCP tools

These are the ones that come up most. For every tool on the server, see [every tool the MCP server gives your AI](/products/pro/integrations/mcp-server/tool-list/).
