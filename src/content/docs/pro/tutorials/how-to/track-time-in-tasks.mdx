...
description: Tallyfy enables time tracking within workflows by adding number-validated
  form fields for hours and minutes to each task so assignees can record their actual
  effort spent rather than relying solely on elapsed-time timestamps which are often
  skewed by holidays and external delays and multitasking.
