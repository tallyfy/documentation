...
description: Root Cause Analysis techniques like the 5 Whys and Fishbone Diagrams
  help identify fundamental reasons behind process problems by systematically exploring
  cause-and-effect relationships through collaborative questioning and visual brainstorming
...
```mermaid
flowchart LR
    Problem["Low customer satisfaction<br/>with support ticket resolution"]
    
    %% Main spine
    Problem --- Spine[ ]
    
    %% Category branches
    Spine --- People["<b>People</b>"]
    Spine --- Process["<b>Process</b>"]
    Spine --- Technology["<b>Technology</b>"]
    Spine --- Environment["<b>Environment/Policy</b>"]
    
    %% People causes
    People --- P1["Insufficient training"]
    People --- P2["High agent turnover"]
    
    %% Process causes
    Process --- Pr1["Too many handoffs"]
    Process --- Pr2["Unclear escalation<br/>procedures"]
    
    %% Technology causes
    Technology --- T1["Slow CRM system"]
    Technology --- T2["Knowledge base<br/>hard to search"]
    
    %% Style the problem as the focal point
    style Problem fill:#F8D7DA,stroke:#DC3545,stroke-width:3px
