---
about_tallyfy: false
description: 'Lean thinking applies Toyota''s waste-elimination framework (TIM WOODS:
  Transportation Inventory Motion Waiting Overproduction Over-processing Defects and
  underused Skills) to office and service work by defining customer value and then
  systematically removing non-value-adding steps through five principles—specifying
  value and mapping the value stream and creating flow and establishing pull and pursuing
  continuous improvement—while tools like Tallyfy embed these ideas directly into
  daily workflows through on-demand process launching'
