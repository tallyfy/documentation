...
description: Tallyfy transforms complex approval workflows into trackable processes
  ...
  routing dynamic assignments real-time status tracking automated reminders collaborative
  communication and organized archiving capabilities.
...
%%{init: {'theme':'forest'}}%%
flowchart TD
    Start([Submit Request]) --> Form["`**Kick-off Form**
Gather Initial Data`"]
    Form --> Validate{Valid<br/>Submission?}
    
    Validate -->|"`No`"| Return["`**Return for**
Corrections`"]
    Return --> Form
    
    Validate -->|"`Yes`"| Amount{Purchase<br/>Amount?}
    
    Amount -->|"`< $10,000`"| Category{Purchase<br/>Category?}
    Amount -->|"`≥ $10,000`"| CFO["`**CFO Approval**
Required`"]
    
    Category -->|"`Software`"| Privacy["`**Vendor Privacy Review**
Assigned to Jane Doe`"]
    Category -->|"`Hardware`"| IT["`**IT Department Review**`"]
    Category -->|"`Services`"| Procurement["`**Procurement Review**`"]
    
    Privacy --> Manager["`**Manager Approval**`"]
    IT --> Manager
    Procurement --> Manager
    CFO --> Manager
    ...
    Decision -->|"`Yes`"| Archive["`**Archive &**
Tag Process`"]
    Decision -->|"`No`"| Notify["`**Notify Requester**
of Rejection`"]
    ...
    Manager -.->|Overdue| Reminder["`**Send Automated**
Reminder`"]
    Reminder -.-> Manager
    
    click Form "/products/pro/tutorials/how-to/build-effective-forms/" "Learn about form building"
    click Manager "/products/pro/documenting/templates/automations/index/" "Configure automations"
    click Archive "/products/pro/tracking-and-tasks/tracker-view/organize-processes/index/" "Organize completed processes"
```
...
<LinkTitleCard header="<b>Examples > Multi-level approval loops</b>" href="/products/pro/documenting/templates/automations/examples/multi-level-approval-loops/" > This guide explains how to create multi-level approval workflows in Tallyfy that automatically reopen previous steps when rejections occur ensuring quality control while maintaining efficient workflow progression through sequential or parallel approval chains. </LinkTitleCard>
</CardGrid>