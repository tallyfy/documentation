...
%%{init: {'theme':'forest'}}%%
flowchart TD
    Start([Submit Request]) --> Form[Kick-off Form<br/>Gather Initial Data]
    Form --> Validate{Valid<br/>Submission?}
    
    Validate -->|No| Return[Return for<br/>Corrections]
    Return --> Form
    
    Validate -->|Yes| Amount{Purchase<br/>Amount?}
    
    Amount -->|< $10,000| Category{Purchase<br/>Category?}
    Amount -->|≥ $10,000| CFO[CFO Approval<br/>Required]
    
    Category -->|Software| Privacy[Vendor Privacy Review<br/>Assigned to Jane Doe]
    ...
    Decision -->|Yes| Archive[Archive &<br/>Tag Process]
    Decision -->|No| Notify[Notify Requester<br/>of Rejection]
    
    Archive --> End([Complete])
    Notify --> End
    
    %% Add reminder automation
    Manager -.->|Overdue| Reminder[Send Automated<br/>Reminder]
    Reminder -.-> Manager
    
    style Start fill:#E8F4FF,stroke:#0066CC
    style Archive fill:#D4EDDA,stroke:#00AA55
    style Notify fill:#F8D7DA,stroke:#DC3545
    style CFO fill:#FFF3CD,stroke:#FFC107
    
    click Form "/products/pro/tutorials/how-to/build-effective-forms/" "Learn about form building"
    click Manager "/products/pro/documenting/templates/automations/" "Configure automations"
    click Archive "/products/pro/tracking-and-tasks/tracker-view/organize-processes/" "Organize completed processes"
```
...
<LinkTitleCard header="<b>Examples > Multi-level approval loops</b>" href="/products/pro/documenting/templates/automations/examples/multi-level-approval-loops/" > This content explains how to build multi-level approval workflows in Tallyfy where rejections automatically reopen previous steps ensuring quality control while maintaining process flow efficiency through status actions and visibility rules. </LinkTitleCard>
</CardGrid>