...
flowchart TB
    subgraph "Initial Request"
        ...
        Form["`**Kick-off Form**
        Gather Initial Data`"]
        Validate{"`Valid<br/>Submission?`"}
        Return["`**Return for**
        Corrections`"]
    end
    
    subgraph "Amount Check"
        Amount{"`Purchase<br/>Amount?`"}
        CFO["`**CFO Approval**
        Required`"]
    end
    
    subgraph "Category Review"
        Category{"`Purchase<br/>Category?`"}
        Privacy["`**Vendor Privacy Review**
        Assigned to Jane Doe`"]
        IT["`**IT Department Review**`"]
        Procurement["`**Procurement Review**`"]
    end
    
    subgraph "Final Approval"
        Manager["`**Manager Approval**`"]
        Decision{"`Approved?`"}
        Reminder["`**Send Automated**
        Reminder`"]
    end
    ...
    Amount -->|"`< $10,000`"| Category
    Amount -->|"`≥ $10,000`"| CFO
    
    Category -->|"`Software`"| Privacy
    Category -->|"`Hardware`"| IT
    Category -->|"`Services`"| Procurement
...
<LinkTitleCard header="<b>Examples > Multi-level approval loops</b>" href="/products/pro/documenting/templates/automations/examples/multi-level-approval-loops/" > Multi-level approval workflows with automatic rejection handling allow you to build complex approval chains where rejected items automatically bounce back to the appropriate step for fixes while using visibility rules and status actions to maintain quality control without stopping the entire process. </LinkTitleCard>
</CardGrid>