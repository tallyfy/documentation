---
description: HSTS (HTTP Strict Transport Security) is a security protocol that forces
  browsers to use only encrypted HTTPS connections with Tallyfy and the platform is
  preloaded into major browsers meaning users receive protection from eavesdropping
  and man-in-the-middle attacks before they even visit the site for the first time.
...
These aren't just arbitrary rules. Each requirement closes a specific security gap. Together, they create an airtight system that keeps your data safe. Proper HSTS implementation significantly reduces the risk of man-in-the-middle attacks.
