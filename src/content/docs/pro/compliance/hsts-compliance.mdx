...
description: HSTS (HTTP Strict Transport Security) forces browsers to use only encrypted
  HTTPS connections with Tallyfy, which is preloaded into major browsers so users
  are protected from eavesdropping and man-in-the-middle attacks before their first
  visit.
