...
There's a practical workaround you can set up right now - use an [expiring task](/products/pro/tracking-and-tasks/tasks/what-types-of-tasks-can-i-create-with-tallyfy/) as an automatic trigger when a deadline passes. Here's the idea: an expiring step auto-completes when its deadline hits, and that completion event fires the [automation](/products/pro/documenting/templates/automations/) engine. You can then chain any action you want - like adding an assignee to the real task.
...
3. Name it something descriptive like "Overdue trigger for [Task Name]"
...
5. Go to **Automations** and create a rule:
   ...
   - **THEN**: Select **"Add assignees"** → choose the person or group → set **"Apply To"** to the actual task that needs the new assignee

When the deadline passes, the expiring step auto-completes, the automation fires, and the assignee gets added to your real task. No manual intervention needed.

**Things to keep in mind**

- The expiring step doesn't require anyone to do anything - it sits quietly until the deadline passes
- If someone acknowledges the expiring step before the deadline, the "is expired" condition won't fire - useful as a way to cancel the escalation
- You can use "Replace assignees" instead of "Add assignees" if you want to fully reassign rather than just add someone
- Form field changes (like checking a checkbox) without completing the step won't trigger automations - the expiring task approach sidesteps this because it completes automatically
...
See our guide on [how to avoid task debt](/products/pro/tutorials/how-to/how-to-avoid-task-debt/) for detailed strategies.
