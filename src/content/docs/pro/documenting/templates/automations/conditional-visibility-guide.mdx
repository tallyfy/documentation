...
description: Tallyfy lets you show or hide entire tasks in a workflow based on form
  field values using IF-THEN rules with AND/OR logic so that each process run only
  reveals the steps relevant to a user's specific selections and answers.
