---
description: Conditional visibility in Tallyfy allows you to dynamically show or hide
  entire tasks based on form field selections using IF-THEN automation rules with
  AND/OR logic to create adaptive workflows where only relevant tasks appear to users.
...
2. **Add detailed kick-off form** with filtering dropdowns:
