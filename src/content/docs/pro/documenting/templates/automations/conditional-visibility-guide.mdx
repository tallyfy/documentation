---
about_tallyfy: true
description: Tallyfy lets you show or hide entire workflow tasks based on form field
  values using IF-THEN rules with AND/OR logic so that a single template can handle
  multiple process variations instead of maintaining separate templates for each path.
