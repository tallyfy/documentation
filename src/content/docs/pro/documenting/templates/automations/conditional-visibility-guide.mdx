...
description: Tallyfy lets you show or hide entire workflow tasks based on form field
  values using automations with AND/OR logic. One template can handle multiple process
...
Show or hide entire tasks based on form field values. Someone fills out a field, Tallyfy checks the conditions you set[^2], and tasks appear or stay hidden. You can combine conditions with AND/OR logic.
...
You'll need a Pro plan (or active trial), plus a [template](/products/pro/documenting/templates/) with form fields and [automations](/products/pro/documenting/templates/automations/).
...
2. Reference these fields in automations throughout the template
...
- Set the If and the Then at the slide level
