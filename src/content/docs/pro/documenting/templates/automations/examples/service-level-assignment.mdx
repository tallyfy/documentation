---
about_tallyfy: true
description: Tallyfy automations use IF-THEN rules on kick-off form fields to automatically
  route tasks to the right team members based on customer service tiers so that enterprise
  clients get senior specialists while free-tier users go to general support groups
  and each customer receives the response speed and expertise level they are paying
  for.
