---
about_tallyfy: true
description: Tallyfy lets you build multi-level approval chains where rejections at
  any stage automatically reopen earlier steps for corrections using approve/reject
  task types and status action rules that handle sequential sign-offs through managers
  and executives while also supporting conditional routing based on field values and
  parallel approvals across multiple teams simultaneously.
