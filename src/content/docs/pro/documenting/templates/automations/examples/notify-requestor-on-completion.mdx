---
about_tallyfy: true
description: Tallyfy lets you capture a process requestor's identity through an Assignee
  Picker field on the kick-off form and then automatically notify them when the process
  finishes by combining an Email Auto-Send step with an automation rule that dynamically
  assigns that notification to the original requestor.
