...
description: Automatically notify the person who launched a process when it completes
  using an Assignee Picker field and an Email Auto-Send automation.
...
Capture who launched a process using an Assignee Picker field, then use an automation to send them a notification when it's done. The person who started the process always knows the outcome - no chasing required.

### Common use cases

- **Service requests** - let employees know their IT, HR, or facilities requests are done
- **Approval workflows** - tell requestors whether they've been approved or rejected
...
1. **Capture the requestor identity**
   ...
   Add an **Assignee Picker** field to your [kick-off form](/products/pro/launching/triggers/kick-off-forms/):
...
3. **Configure the automation**
   ...
   - **THEN action**: Choose ["Replace Assignees"](/products/pro/documenting/templates/automations/actions/asignment-actions/)
...
3. Set up automations to route each notification correctly
...
2. Connect it to [middleware](/products/pro/integrations/middleware/) (Zapier, Make, Power Automate) to:
...
- Confirm the step type is "Email Auto-Send" - it shouldn't be a regular task
...
- Referenced fields must be filled before the notification step fires - they can't be empty
...
## Related articles
<CardGrid>
<LinkTitleCard header="<b>Examples > Automatically assign a step to the original process requestor</b>" href="/products/pro/documenting/templates/automations/examples/assign-step-to-requestor/" > Capture who started a process using an Assignee Picker field, then auto-assign a final step to... </LinkTitleCard>
