---
about_tallyfy: true
description: Organize your Tallyfy template library with folders, naming conventions,
  tags, and step grouping so teams can find and run the right process fast. Includes
  version control and audit cycles to keep templates accurate.
...
To completely hide a template from Standard and Light users[^1] in the Templates section, restrict **ALL THREE** permissions (EDIT, READ, and LAUNCH) for those users. If any single permission remains, the template still appears in their Templates view.
:::
...
[^1]: View-only users who can't complete tasks but can see process progress
