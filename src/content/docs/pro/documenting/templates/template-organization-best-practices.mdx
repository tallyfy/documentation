---
description: Template organization in Tallyfy dramatically improves workflow efficiency
  through strategic folder structures naming conventions tagging systems governance
  frameworks and maintenance schedules that ensure templates remain accessible accurate
...
- **Task-based grouping**: Combine related actions into single steps with checklists
  ...
  - Use: 1 step called "Prepare documents" with a 10-item checklist field
...
To completely hide a template from Standard and Light users in the Templates section, you must restrict **ALL THREE** permissions (EDIT, READ, and LAUNCH) for those users. If any single permission remains, the template will still appear in their Templates view.
:::
