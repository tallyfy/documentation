---
about_tallyfy: true
description: Tallyfy turns static BPMN diagrams into real running workflows by mapping
  standard patterns like sequential flows, gateways, and loops to plain automations
  and steps anyone on your team can build and launch without technical training.
...
|-------------------------------|---------------------------------------------|
| Sequential Flow               | Steps run one after another - use automations to enforce order |
| Parallel Split (AND)          | Multiple steps are active at the same time (default behavior) |
| Exclusive Gateway (XOR)[^1]       | Automations show/hide steps so only one path runs |
| Inclusive Gateway (OR)[^2]        | Automations show/hide steps so multiple paths can run |
| Synchronization               | Automations wait for multiple steps to complete before triggering |
| Structured Loop               | Reopen action lets you repeat steps within a process |
| Deferred Choice               | Automations conditionally assign tasks to different people |
...
4. **Build your workflow** - add steps, assign them to the right people, and set up automations
5. **Add the details** - add [form fields](/products/pro/tracking-and-tasks/tasks/what-are-form-fields-in-tallyfy/), set the conditions that decide each path, and add [automations](/products/pro/documenting/templates/automations/)
