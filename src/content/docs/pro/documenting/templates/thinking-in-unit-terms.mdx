---
about_tallyfy: true
description: Tallyfy templates should be designed to handle one single item at a time
  — such as one employee or one invoice — rather than batches because this approach
  provides clear task ownership and precise tracking while allowing you to launch
  hundreds of parallel instances that each move independently without one delayed
  item blocking everything else.
