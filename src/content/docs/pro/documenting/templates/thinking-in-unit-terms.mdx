---
about_tallyfy: true
description: Tallyfy templates should be built to handle one single item at a time
  — like one employee or one invoice — rather than batches because this approach creates
  clear task ownership and precise tracking while allowing you to launch hundreds
  of parallel instances that each progress independently without one delayed item
