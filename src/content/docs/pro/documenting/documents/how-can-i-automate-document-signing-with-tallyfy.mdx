...
sequenceDiagram
    ...
    participant M as Middleware<br/>(Zapier/Make)
    participant E as E-Signature<br/>(DocuSign/HelloSign)
    participant S as Signer
    
    Note over User,S: Automated Document Signing Flow
    
    User->>T: Completes workflow step
    T->>T: Collects form data<br/>(name, email, terms)
    T->>M: POST webhook with data
    Note right of M: Webhook URL configured<br/>in Tallyfy step
    
    M->>M: Maps Tallyfy fields to<br/>document template fields
    M->>E: Create signature request<br/>from template
    ...
    Note right of T: Updates task status<br/>automatically
```
