...
description: Tallyfy automates document signing by connecting to e-signature services
  through middleware tools that trigger automatic signature requests when workflow
  steps are completed saving teams 15-20 minutes per document.
...
sequenceDiagram
 ...
 participant M as Middleware (Zapier/Make)
 participant E as E-Signature (DocuSign/HelloSign)
 participant S as Signer
 
 Note over User,S: Automated Document Signing Flow
 
 User->>T: Completes workflow step
 T->>T: Collects form data (name, email, terms)
 T->>M: POST webhook with data
 Note right of M: Webhook URL configured in Tallyfy step
 
 M->>M: Maps Tallyfy fields to document template fields
 M->>E: Create signature request from template
 E->>S: Send document for signature
 S->>E: Signs document
 
 Note over M,T: Optional Return Path
 E->>M: Document signed notification
 M->>T: POST /api/v1/tasks/{id}/complete
 Note right of T: Updates task status automatically
```
