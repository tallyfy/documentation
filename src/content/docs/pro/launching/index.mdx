---
description: Tallyfy transforms reusable workflow templates into active trackable
  processes through various launch methods including manual triggers API calls email
  triggers web forms magic links and task completion chains while maintaining process
  independence from template changes and allowing customization during launch.
...
graph TD
    ...
    Template --> Methods["`**Choose Launch Method**`"]
    
    Methods --> Manual["`**1. Manual Launch**
    User clicks Run`"]
    
    Manual --> API["`**2. API/Webhook**
    System trigger`"]
    
    API --> Task["`**3. Task Completion**
    Chain launch from other process`"]
    
    Task --> Form["`**4. Web Form**
    Public form submission`"]
    
    Form --> Email["`**5. Email Trigger**
    Send to designated address`"]
    
    Email --> Magic["`**6. Magic Link**
    Special URL for external users`"]
    
    Magic --> Process["`**Active Process**
    ...
    click Form "/products/pro/launching/how-to-launch-a-tallyfy-process-from-a-webform/" "Web form launching"
    click Email "/products/pro/launching/triggers/via-email/" "Email triggers"
    click Magic "/products/pro/launching/triggers/magic-links/" "Magic links"
    click Task "/products/pro/launching/launch-process-when-task-is-completed/" "Task completion triggers"
```
