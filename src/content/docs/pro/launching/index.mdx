...
```mermaid
flowchart TD
    Template[Template<br/>Reusable Blueprint] --> Launch{Choose Launch<br/>Method}
    
    Launch -->|Manual| Manual[User clicks<br/>Launch button]
    Launch -->|Automated| API[API/Webhook<br/>Trigger]
    Launch -->|Cascade| Cascade[Task completion<br/>in other process]
    Launch -->|Form| WebForm[Web form<br/>submission]
    Launch -->|Email| Email[Email to<br/>process address]
    Launch -->|External| Magic[Magic link<br/>no login required]
    
    Manual --> Customize{Customize<br/>Instance?}
    API --> Customize
    Cascade --> Customize
    WebForm --> Customize
    Email --> Customize
    ...
    Customize -->|Yes| Custom[Set name, assignments,<br/>deadlines, guests,<br/>kick-off data]
    Customize -->|No| Default[Use template<br/>defaults]
    
    Custom --> Process[Active Process<br/>Independent Instance]
    Default --> Process
    
    style Template fill:#E8F4FF,stroke:#0066CC
    style Process fill:#D4EDDA,stroke:#00AA55
    style Launch fill:#FFF3CD,stroke:#FFC107
