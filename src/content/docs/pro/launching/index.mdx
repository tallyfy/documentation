...
graph TD
    ...
    Template --> Choice{"`**Choose ANY
    Launch Method**`"}
    
    Choice -.->|"`Option`"| Manual["`**Manual**
    User clicks Run`"]
    
    Choice -.->|"`Option`"| API["`**API/Webhook**
    System trigger`"]
    
    Choice -.->|"`Option`"| Task["`**Task Done**
    Chain launch`"]
    
    Choice -.->|"`Option`"| Form["`**Web Form**
    Public submit`"]
    
    Choice -.->|"`Option`"| Email["`**Email**
    Send to address`"]
    
    Choice -.->|"`Option`"| Magic["`**Magic Link**
    Special URL`"]
