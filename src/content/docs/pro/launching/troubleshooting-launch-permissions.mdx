---
about_tallyfy: true
description: Tallyfy requires Standard members to have explicit launch permission
  on templates and common issues include permission denials on launch and auto-assignment
  of launchers to unassigned steps and Magic Link failures when required kick-off
  fields are missing and dropdown field mapping problems in Power Automate that need
  JSON parsing to extract the correct text or ID values.
...
:::note
You can't launch a draft template. If dual versioning's enabled, the template must be in a published state.
:::
