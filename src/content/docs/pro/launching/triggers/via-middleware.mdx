...
sequenceDiagram
    autonumber
    participant EA as External App<br/>(CRM/Form/Database)
    participant MW as Middleware<br/>(Zapier/Make/Power Automate)
    participant TF as Tallyfy

    Note over EA,TF: Automated Process Launching Flow
    
    EA->>EA: Event occurs<br/>(e.g., form submitted)
    EA-->>MW: Trigger notification
    MW->>MW: Detect trigger event
    MW->>MW: Map data fields<br/>to Tallyfy format
    MW->>TF: POST /api/processes/launch<br/>with mapped data
    TF->>TF: Create process instance
    TF->>TF: Populate kick-off form
    TF-->>MW: Process ID & status
    MW-->>EA: Confirmation (optional)
    
    Note over TF: Process now running<br/>with external data
```
