...
description: Middleware platforms enable automated launching of Tallyfy processes
  by acting as bridges between external applications and Tallyfy workflows where trigger
  events in other systems automatically initiate specific process templates with mapped
  data transfer for seamless business automation.
...
sequenceDiagram
 autonumber
 participant EA as External App (CRM/Form/Database)
 participant MW as Middleware (Zapier/Make/Power Automate)
 participant TF as Tallyfy

 Note over EA,TF: Automated Process Launching Flow
 
 EA->>EA: Event occurs (e.g., form submitted)
 EA-->>MW: Trigger notification
 MW->>MW: Detect trigger event
 MW->>MW: Map data fields to Tallyfy format
 MW->>TF: POST /api/processes/launch with mapped data
 TF->>TF: Create process instance
 TF->>TF: Populate kick-off form
 TF-->>MW: Process ID & status
 MW-->>EA: Confirmation (optional)
 
 Note over TF: Process now running with external data
```
