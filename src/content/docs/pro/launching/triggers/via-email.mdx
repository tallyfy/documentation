---
description: Tallyfy allows users to launch specific processes by sending emails to
  unique template addresses with the email subject becoming the process name and optional
  kick-off fields capturing email content and attachments automatically.
...
*   The email address you send *from* must belong to an active [member](/products/pro/documenting/members/) in your Tallyfy<sup>®</sup> organization.

### Launching processes by sending an email

Tallyfy<sup>®</sup> lets you start a specific [process](/products/pro/tracking-and-tasks/processes/) by sending an email to a special address. This is useful for starting workflows without logging into Tallyfy<sup>®</sup>, especially for users on the go.
