---
description: Email triggers in Tallyfy allow users to launch workflows by sending
  emails to unique template-specific addresses which automatically start processes
  using the email subject as the process name and can capture email content and attachments
  through special kick-off form fields while requiring the sender to be an active
  organization member.
...
```mermaid
flowchart TD
    Email[Email to Address] --> Validate{Valid Sender?}
    
    Validate -->|No| Reject[Rejected]
    Validate -->|Yes| Check{Required Fields?}
    
    Check -->|Yes| Fail[Cannot Launch]
    Check -->|No| Launch[Launch Process]
    
    Launch --> Notify[Process Started]
    
    click Validate "/products/pro/documenting/members/index/" "Learn about member management"
    click Check "/products/pro/launching/triggers/kick-off-forms/" "Understanding kick-off forms"
```
