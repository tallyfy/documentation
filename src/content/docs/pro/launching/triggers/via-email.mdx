---
about_tallyfy: true
description: Tallyfy lets you launch a workflow process by sending an email to a template's
  unique address — verifying the sender is an active member and using the subject
  line as the process name — which is ideal for mobile users or external systems but
  works best with templates that have no required kick-off fields.
