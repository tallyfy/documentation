...
<Steps>
1. **Check who sent it.** Only active members of your organization can launch a process or add a comment by email. A reply sent from a personal address, or from someone who's been removed, won't be accepted.
2. **Check the address.** Reply to the notification directly rather than typing the address by hand. The `bot+[TaskID]-comment-task@tallyfy.com` and `bot+[TemplateID]-launch@tallyfy.com` addresses have to match exactly.
3. **Check the template's kick-off fields.** If any kick-off field is marked as required, email launching won't work for that template. Make those fields optional, or launch it another way.
4. **Give it a few minutes, then look at the task.** Your reply shows up as a comment on the task, not as a reply in your inbox.
</Steps>

If you've checked all four and your email still hasn't arrived, contact Tallyfy support and include the time you sent it and the address you sent it to. Those two details let us trace the specific message.

You won't always get a bounce message when an emailed reply doesn't make it through. If something matters, it's worth confirming in Tallyfy rather than assuming the email arrived.
