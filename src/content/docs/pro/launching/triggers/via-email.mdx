---
about_tallyfy: true
description: Tallyfy lets you launch a process by sending an email to a template's
  unique address — useful for mobile users or external systems — where the email subject
  becomes the process name and the body and attachments can be captured into form
  fields as long as the sender is an active organization member and the template has
  no required kick-off fields.
