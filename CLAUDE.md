...
- All frontmatter descriptions must end with a period. For their LENGTH see the single
  rule under "Article Structure Rules" below, and do not restate a number here: a length
  quoted in two places is how this file came to state two different ones
...
**📁 COMPLETE DOCUMENTATION STRUCTURE**: See `DOCUMENTATION_STRUCTURE.md` for comprehensive organization guide including 712 .mdx files across 148 directories, search strategies, and file patterns.
...
- **Tallyfy Pro** (Primary focus): Create, launch, track and improve repeatable business processes
  - Location: `/src/content/docs/pro/` (686 files - 90% of all documentation)
  - Core areas: documenting/ (79 files), tracking-and-tasks/ (51 files), integrations/ (291 files)
...
1. **Headers** (DO NOT EDIT except for new articles):
   ...
   - **description** field: Auto-generated by CI/CD, but if writing manually:
     - **Length**: 200-350 characters. This is the AUTHORITATIVE statement of the range.
       `scripts/generate-snippets.py` repeats it in the prompt it hands the model and cites
       this section as its source; anywhere else that quotes a number is a copy to be
       corrected here first.
     - **BOTH bounds are ADVISORY when you write a description by hand, which is what this
       bullet is about. Nothing validates description length on any path a human edit takes.**
       `scripts/markdown-lint.py` never looks at `description` (0 occurrences of the word,
       against 3 `def` in the same file). The 350 truncation in `post_process_description`
       bounds only what the GENERATOR writes: it is reached from `process_file` alone, over
       the `--files` list that `documentation-pipeline.yml` builds with
     ...
     - **Why 200 rather than 150**: 150 was stated twice and justified nowhere, and the
       ...
       `scripts/generate-snippets.py` actually asks the model for, and it is what the corpus
       is written to. Measured on `staging` at `bbfbdd99c`, 2026-08-31, over the 579
       non-changelog articles carrying a description: min 122, p25 199, median 225, p75 245,
       max 402. **147 sit below 200, 5 below 150, and 5 above 350.** Those are legacy. Do NOT
       bulk-rewrite them to satisfy this rule; they belong to the content simplification pass.
