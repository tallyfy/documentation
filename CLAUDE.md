...
1. **Headers** (DO NOT EDIT except for new articles):
   ...
   - **description** field: Auto-generated by CI/CD, but if writing manually:
     - **Length**: 200-350 characters. **This is the only place this repository states that
       range.** Anything else quoting a number is a copy, and copies are what made this file
       contradict itself. **The 350 ceiling is ENFORCED**: `post_process_description` in
       `scripts/generate-snippets.py` truncates at it. **The 200 floor is ADVISORY**, because
       nothing validates it. `scripts/markdown-lint.py` checks `id`, `sidebar` and
       `sidebar.order` and never looks at `description` at all (0 occurrences of the word,
       against 3 `def` in the same file, measured 2026-08-31).
