---
id: 2337c2a35ba16a016928c0a58d8d5e51
snippet: A template serves as a blueprint for launching multiple processes where each
  launched instance operates independently with its own unique tasks and tracking
  capabilities while modifications to the template only affect future launches.
title: Launching
weight: 2
---

## Launching is ridiculously fun!

Launching a process involves grabbing a copy of template and running it for a specific situation.

You are taking the template (recipe) and copying it for a specific scenario at the moment you launch it.

One template can have many processes. An example of a template is "Client Onboarding" and an example of a process that uses that template is "Coca Cola". In other words - we're running the process "Coca Cola" using the template "Client Onboarding".

If you make changes to a template, they only reflect on the next process you launch.


import PageTree from '@/components/PageTree.astro';

<PageTree />

import { CardGrid, LinkTitleCard} from "~/components";

## Related articles
<CardGrid>
<LinkTitleCard header="<b>Tutorials > Launch a process</b>" href="/products/pro/tutorials/your-first-process/" > Process launching involves naming, configuring kick-off forms, assigning tasks, setting deadlines, and tracking progress through the Tracker view after selecting the Launch button from the Templates section. </LinkTitleCard>
<LinkTitleCard header="<b>Launching > Launch another process when a task is completed</b>" href="/products/pro/launching/launch-process-when-task-is-completed/" > A workflow automation feature that enables launching subsequent processes or injecting tasks when specific steps are completed, with automatic tagging to maintain traceable connections between parent and child processes. </LinkTitleCard>
<LinkTitleCard header="<b>Tracking And Tasks > More about processes</b>" href="/products/pro/tracking-and-tasks/processes/" > A process is a unique instance created from a template that can be launched multiple times, named distinctively, and contains specific tasks, steps, and audit trails for tracking individual cases or scenarios. </LinkTitleCard>
<LinkTitleCard header="<b>Launching > Triggers</b>" href="/products/pro/launching/triggers/" > Processes can be initiated both manually and automatically through various triggers like emails, magic links, or API integrations, eliminating manual data entry by allowing direct data transfer from external systems. </LinkTitleCard>
</CardGrid>