...
snippet: Files can be attached to task fields in Tallyfy through API calls either
  by uploading local files using a POST request or by linking external files via a
  PUT request with specific JSON payloads and authentication tokens.
