---
id: abc20671dbf3fadef1f88c1e75083ec3
snippet: Middleware enables document generation from Tallyfy form fields by connecting
  steps to services like Zapier or Microsoft Power Automate which then sends form
  data to WebMerge for automatic document creation and email distribution.
title: Send data to document creation systems
weight: 13
---

## Generate documents via form fields using other document generation systems

This article will show you how to use middleware to generate documents from form fields in Tallyfy.

To start, you can connect a step in a Tallyfy template to Zapier or other middleware like Microsoft Power Automate. This will allow you to send form contents from previous steps to WebMerge.

Once the form contents are sent to WebMerge, you can then have the document automatically sent to a designated email address.

<lite-vimeo videoid="311699511"></lite-vimeo>

import { CardGrid, LinkTitleCard} from "~/components";

## Related articles
<CardGrid>
<LinkTitleCard header="<b>Middleware > Stream data to a spreadsheet</b>" href="/products/pro/integrations/middleware/how-can-i-export-form-fields-from-tallyfy-to-spreadsheet/" > Form field data from Tallyfy can be streamed directly to spreadsheets using middleware services like Zapier or Microsoft Power Automate for efficient data collection and storage. </LinkTitleCard>
<LinkTitleCard header="<b>Launching > Launch a process from a web form</b>" href="/products/pro/launching/how-to-launch-a-tallyfy-process-from-a-webform/" > Tallyfy enables users to efficiently launch processes and automatically populate fields through a web form integration by sharing a unique URL with customizable query parameters for field values. </LinkTitleCard>
<LinkTitleCard header="<b>Tasks > Types of form fields</b>" href="/products/pro/tracking-and-tasks/tasks/what-are-form-fields-in-tallyfy/" > Tallyfy offers diverse form field options including short text, long text, dropdown lists, checklists, radio buttons, date/time selectors, file uploads, tables, and assignee pickers, each with customizable settings for enhanced data collection and workflow management. </LinkTitleCard>
<LinkTitleCard header="<b>Features > Structure intake</b>" href="/products/pro/tutorials/features/structure-intake/" > Transform standalone forms into trackable workflows by integrating them with automated processes that ensure accountability, real-time status updates, and seamless data transfer between steps and systems. </LinkTitleCard>
</CardGrid>